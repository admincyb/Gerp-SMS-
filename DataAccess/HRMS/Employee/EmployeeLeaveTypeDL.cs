...
{
    public class EmployeeLeaveTypeDL
    {
        ...
        public static DataTable GetEmpLeaveTypeList(int? EmpLeavePk, int EmpPk, int Active, int bizUnit)
        {
            ...
            colParameters = new DBService.Parameters[] 
                {
                    new DBService.Parameters(Parameters.P_ELV_PK, EmpLeavePk.HasValue? EmpLeavePk : (object)DBNull.Value),
                    new DBService.Parameters( Parameters.P_ELV_EMPLOYEE, EmpPk),  
                    new DBService.Parameters(Parameters.P_ACTIVE, Active),                                      
                    new DBService.Parameters(Parameters.BIZUNIT, bizUnit),
                };
            dtLeaveTypes = dbService.DataAdapter(CommandType.StoredProcedure, Procedures.SPHRM_EMP_LEAVE_TYPE_DTL_GET_KV, colParameters).Tables[0];
            return dtLeaveTypes;
        }
        ...
        public static int SaveEmployeeLeaveTypes(string xmlDoc)
        {
            ...
            int rows = dbService.ExecuteNonQuery(CommandType.StoredProcedure, Procedures.SPHRM_EMP_LEAVE_TYPE_DTL_SAVE, colParameters);
            int result = Convert.ToInt32(((IDataParameter)dbService.oCommand.Parameters[GTIService.Constants.Common.Parameters_Common.RETVAL]).Value);
            return result;
        }
        ...
        public static DataTable GetEmpLeaveCreditDetails(int empPk, int leaveType, int bizUnit)
        {
            ...
            colParameters = new DBService.Parameters[] 
                {
                    new DBService.Parameters(Parameters.P_LEAVE_TYPE, leaveType > 0? leaveType : (object)DBNull.Value),
                    new DBService.Parameters( Parameters.P_EMP_PK, empPk),  
                    new DBService.Parameters(Parameters.BIZUNIT, bizUnit),
                };
            dtLeaveTypes = dbService.DataAdapter(CommandType.StoredProcedure, Procedures.SPHRM_EMP_LEAVE_CREDIT_GET, colParameters).Tables[0];
            return dtLeaveTypes;
        }
