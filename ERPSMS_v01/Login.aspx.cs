...
namespace LatexERPV2
{
    public partial class Login : System.Web.UI.Page
    {
        ...
        {
            string s = GetGlobalResourceObject("ConfigurationsRes", "AutoCompleteLimit").ToString();
        ...
        /// <returns></returns>
        private static bool IsValidToken(string UserID, string token)
        {

            ERP.Utilities.CryptoServices crypto = new ERP.Utilities.CryptoServices();
            string passWord = HttpUtility.HtmlEncode(crypto.DecryptString(token, System.Configuration.ConfigurationManager.AppSettings["salt"]));
            BusinessLogic.Login login = new BusinessLogic.Login();
            var uLogin = login.Authenticate(UserID, passWord);
            if (uLogin != null)
                return true;
            else
                return false;
