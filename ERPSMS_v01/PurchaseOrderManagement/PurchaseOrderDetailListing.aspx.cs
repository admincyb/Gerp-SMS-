...
{
    public partial class PurchaseOrderDetailListing : ERP.Store.UI.MyBasePage
    {
        ...
        private void PageActionHandler()
        {
            try
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    string Type = POH_MENU_TYPE.Value.ToString();
                   
                    btnAdd.Visible = BusinessLogic.CommonManagement.CommonManagement.GetInitialTaskPermission(currentUser.PKUser, FillProcessId(Convert.ToInt32(Type)));
                    hdfAppType.Value = ApplicationType.PO;
                    hdfAppSubType.Value = string.Empty;
                    GetUserRights();
                    ConfigurationSettings();

                    FillProcessId(Convert.ToInt32(Type));
                    ...
                    {
                        grdPOList.HeaderRow.Cells[3].Visible = false;
                    ...
                    if (GetGlobalResourceObject("ConfigurationsRes", "UseEmployeeAsPOCreator").ToString() == "1")
                    {
                        if (grdPOList.Rows.Count > 0)
                        {
                            grdPOList.HeaderRow.Cells[3].Visible = true;
                            grdPOList.Columns[3].Visible = true;
                        }                     
                    }
                }
            }
            ...
            try
            {
                switch (type)
                {
                    ...
                    case ControlsEnum.DEFAULT:
                        ...
                        gridParam.POH_MENU_TYPE = Request.QueryString["TYPE"] == null ? 0 : Convert.ToInt32(Request.QueryString["TYPE"]);

                        processId = GetNullableInt(hdfProcId.Value) ?? 0;
                        pageUrl = Resources.PageURL.PurchaseOrderCreation; // "/PurchaseOrderManagement/PurchaseOrderGenerate.aspx?TYPE=1"; 
                        if (Request.QueryString["TYPE"] != null)
                            pageUrl = Resources.PageURL.PurchaseOrderGenerate + "?TYPE=" + Request.QueryString["TYPE"].ToString();
                        ...
                        int poType = 0, pohPOCategory=0;
                        if (hdfPRtypeEnabled.Value == "1")//In the case of EKK
                        {
                            pohPOCategory = string.IsNullOrEmpty(ddlPOType.SelectedValue) ? 0 : Convert.ToInt32(ddlPOType.SelectedValue);
                        }
                        else
                        {
                            poType = string.IsNullOrEmpty(ddlPOType.SelectedValue) ? 0 : Convert.ToInt32(ddlPOType.SelectedValue);
                        }
                        if ((hdfInvNo.Value != "") && !string.IsNullOrEmpty(txtInvNo.Text) && (txtInvNo.Text != "Select/Type"))
                            gridParam.InvNo = txtInvNo.Text;
                        else
                            gridParam.InvNo = string.Empty;
                        dsPageData = DataAccess.PurchaseOrderManagement.PurchaseOrderGeneration.GetPurchaseOrderList(gridParam, currentUser.SBUID, pageUrl, filterStatus
                           ,transactionStatus, vendorPk, poNo, prNo, ioNo, reqFor, orderGroup, itemName, cmpPk,poType,pohPOCategory,reqDept);
                    ...
                    case ControlsEnum.TRANSACTIONSTATUS:
                        dtPageData = CommonBL.GetAppStatus("PO", string.Empty);
                        break;
                    #endregion
                    #region Plant
                    ...
                    case ControlsEnum.POTYPES:
                        if (hdfPRtypeEnabled.Value == "1")//In the case of EKK
                        {
                            dtPageData = DataAccess.Administration.Masters.CompanyMasterDA.GetConstantValue(0, 1, 0, 10, 3, currentUser.SBUID);             
                        }
                        else
                        {
        ...
        protected void ActionHandler(object sender, EventArgs e)
        {
            ...
            switch (commonActions)
            {
                ...
                case ActionsEnum.NEW:
                    string RequestURL = GetLocalResourceObject("PurchaseOrderEntryURL").ToString();
                    if (Request.QueryString["TYPE"] != null)
                    {
                        RequestURL += "?TYPE=" + Request.QueryString["TYPE"].ToString();
                        if (Request.QueryString["Sbu"] != null)
                        {
                            RequestURL += "&Sbu=" + Request.QueryString["Sbu"].ToString();
                        }
                    }
                ...
                case ActionsEnum.SHORTCLOSESAVE:
                    ...
                    {
                        poShortClose.CheckFlag = 1; //Check it is a non Stock PO or not
                    }
                    else
                    {
                        poShortClose.CheckFlag = 0; 
                    }                  
                    ...
                    else
                    {
                        ...
                        else if (shortCloseResult == -29)//Non Stock Item .After Shortclosing, this PO will not be available for further transaction.
                        {
                            ...
                            ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "PONotAvailableFurther", "$(document).ready(function(){ShowPONotAvailableFurtherContinue(1);});", true);
                        }
                        else
                        {
            ...
            try
            {
                ...
                if ((sender as GridView).ID == "grdPOList")
                {
                    ...
                    if (e.Row.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        int IsGRNExist = Convert.ToInt32((e.Row.FindControl("hdfGRNExist") as HiddenField).Value); 
                        ...
                        int isConvertedPO = 0;
                        Int32.TryParse((e.Row.FindControl("hdfisConvertedPO") as HiddenField).Value, out isConvertedPO);
                        ...
                        if (isClosePO == 1 && poStatus != 0 && poStatus != 4 && poStatus != 5 && poStatus != 3 && isConvertedPO==0)//0-->Draft,4-->Shortclosed,5-->Closed
                        { 
                        ...
                        if (hdfGRNExistForShortClose.Value == "1" && IsGRNExist == 1)
                            imbShortClose.Visible = false;
                        if (poStatus == 4)
                        {
                            imbEdit.Visible = false;
                ...
                switch (e.CommandName)
                {
                    case "PERFORMACTION":
                        ...
                        if (UserStatus == 1)
                        {
                            ...
                            RequestURL = GetLocalResourceObject("PurchaseOrderEntryURL").ToString() + "?RefID=" + refID;
                        }
                        else if (UserStatus == 2)
                        {
                            //Response.Redirect(GetLocalResourceObject("PurchaseOrderEntryURL").ToString() + "?POID=" + poId);
                            RequestURL = GetLocalResourceObject("PurchaseOrderEntryURL").ToString() + "?POID=" + poId;
                        }
                        if (Request.QueryString["TYPE"] != null)
                        {
                            RequestURL += "&TYPE=" + Request.QueryString["TYPE"].ToString();
                    ...
                    case "VIEW":
                        ...
                        if (UserStatus == 1)
                        {
                            ...
                            RequestURL = GetLocalResourceObject("PurchaseOrderEntryURL").ToString() + "?RefID=" + refID + "&Status=1";
                        }
                        else if (UserStatus == 2)
                        {
                            //Response.Redirect(GetLocalResourceObject("PurchaseOrderEntryURL").ToString() + "?PK=" + poId + "&Status=1");
                            RequestURL = GetLocalResourceObject("PurchaseOrderEntryURL").ToString() + "?PK=" + poId + "&Status=1";
                        }
                        else if (UserStatus == 0)
                        {
                            if (refID == 0)
                    ...
                    case "PRINT":
                        ...
                        {
                            url = GetLocalResourceObject("PurchaseOrderReportURL").ToString() + "?ID=" + poId + "&APPTYPE=" + ApplicationType.POP + "&APPSUBTYPE=0" ;
                        }
                        else if (Request.QueryString["TYPE"] == "4")
                        {
                            url = GetLocalResourceObject("PurchaseOrderReportURL").ToString() + "?ID=" + poId + "&APPTYPE=" + ApplicationType.POPG + "&APPSUBTYPE=0";

                        }
                        else if(Request.QueryString["TYPE"] == "5")
                        {
                            url = GetLocalResourceObject("PurchaseOrderReportURL").ToString() + "?ID=" + poId + "&APPTYPE=" + ApplicationType.POG + "&APPSUBTYPE=0";

                        }
                        else if (Request.QueryString["TYPE"] == "6")
                        {
                            url = GetLocalResourceObject("PurchaseOrderReportURL").ToString() + "?ID=" + poId + "&APPTYPE=" + ApplicationType.POTR + "&APPSUBTYPE=0";

                        }
                        else
                        {
                            url = GetLocalResourceObject("PurchaseOrderReportURL").ToString() + "?ID=" + poId + "&APPTYPE=" + hdfAppType.Value.ToString() + "&APPSUBTYPE=" + hdfAppSubType.Value.ToString();
                    ...
                    case "COMMENTS":
                        ...
                        ucTrxComments.ViewType =Convert.ToInt16(hdfUcrTransCommentsPanelShow.Value);//Convert.ToInt16(BusinessLogic.CommonManagement.CommonManagement.GetInitialTaskPermission(currentUser.PKUser, Convert.ToInt32(hdfProcId.Value)));
        ...
        private int FillProcessId(int Type)
        {
            ...
            string path = "purchaseordermanagement/purchaseordergenerate.aspx?TYPE=" + Type.ToString(); //1";
                ...
                switch (controlType)
                {
                    ...
                    case ControlsEnum.POTYPES:
                        ...
                        if (dtPageData != null && dtPageData.Rows.Count > 0)
                        {
                            if (hdfPRtypeEnabled.Value == "1")//In the case of EKK
                            {                                
                                ddlPOType.DataValueField = GTIService.Constants.Common.Fields.CON_VALUE;
                                ddlPOType.DataTextField = GTIService.Constants.Common.Fields.CON_NAME;
        ...
        private void ConfigurationSettings()
        {
            ...
            hdfGRNExistForShortClose.Value = GetGlobalResourceObject("ConfigurationsRes", "GRNExistForPOShortClose").ToString(); 
            ...
            if (hdfIsShowInvNo.Value == "0")
            {
                ...
                lblReqDept.CssClass = "";
                grdPOList.Columns[11].Visible = false;
            }
        }
        private void GetUserRights()
        {
            #region ShortClose
            string path = GetLocalResourceObject("POShortClosureURL").ToString() + POH_MENU_TYPE.Value;//Purchase Order - Short Closure
            ...
            #endregion
            #region Transaction Comments Entry div Show/Hide
            string pathPOList = GetLocalResourceObject("POListURL").ToString();
            UserRightsBO usrRightsTrnComments = userAuth.GetUserRights(currentUser.PKUser, pathPOList, currentUser.SBUID, currentUser.CurrentDeptPK);
            if (usrRightsTrnComments.Rights.Count > 0)
            {
                for (int i = 0; i < usrRightsTrnComments.Rights.Count; i++)
                {
                    if (usrRightsTrnComments.Rights[i].ActionName == "DISPLAYSHOW" && usrRightsTrnComments.Rights[i].HasActionRight == false)
                    {
                        hdfUcrTransCommentsPanelShow.Value = "0";
                        break;
                    }
                }
            }
        ...
        public void AssignLocalBreadCrumb()
        {
            try
            {
                ...
                {
                    if (this.GetLocalResourceObject("BreadcrumbPackingCov") != null)
                    {
                        string breadCrumb;
                        breadCrumb = this.GetLocalResourceObject("BreadcrumbPackingCov").ToString().Replace(">>", "<label style='font-family:Arial; letter-spacing:-7px; padding:0 7px 0 2px'>&#9658;</label>");
                ...
                else if (Request.QueryString["TYPE"] != null && Request.QueryString["TYPE"].ToString() == "5")
                {
                    if (this.GetLocalResourceObject("BreadcrumbGlove") != null)
                    {
                        string breadCrumb;
                        breadCrumb = this.GetLocalResourceObject("BreadcrumbGlove").ToString().Replace(">>", "<label style='font-family:Arial; letter-spacing:-7px; padding:0 7px 0 2px'>&#9658;</label>");
                        lblBreadCrum.Text = breadCrumb;
                        //Page.Title = GetLocalResourceObject("PackingTitle").ToString();
                    }
                }
