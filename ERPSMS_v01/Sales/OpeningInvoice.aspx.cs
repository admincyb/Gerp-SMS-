...
{
    public partial class OpeningInvoice : ERP.Store.UI.MyBasePage
    {
            ...
            try
            {
                switch (type)
                {
                    ...
                    case ControlsEnum.MISCLIST:
                        ...
                        //    }, currentUser, cusID, InvPk, 0, HttpUtility.HtmlEncode(customer), InvType, txtSCno.Text, Resources.PageURL.Misc.Replace("~", ""), "", Status, Convert.ToInt32(chkBalAmt.Checked), group: (byte)SalesInvoiceGroup.Goods, category: 1);
                        dsPageData = null;
                        if (dsPageData != null)
                        {
                            //string customer = string.IsNullOrEmpty(txtCustomerLIST.Text.Trim()) ? string.Empty : (txtCustomerLIST.Text.Trim() == "Select/Type" ? string.Empty : txtCustomerLIST.Text.Trim());
                ...
                switch (controlType)
                {
                    ...
                    case ControlsEnum.SOINVHEADER:
                        if (SOInvoiceHeaderSession != null)
                        {
                            ...
                            invoiceHeaderObj.ICH_IS_OPENING = 1;
        ...
        private void FillProcessID(int pid)
        {
            ...
            if (pid == 2)
            {
                path = GetLocalResourceObject("SIJPath").ToString().ToLower();
            }
                ...
                switch (commonActions)
                {
                            ...
                            if (invoiceHeaderObj != null && invoiceHeaderObj.OrderDetail != null)
                            {
                                ...
                                if (result > 0) // Success !  redirect to listing page
                                {
                                    ...
                                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowErrorMsg", "ShowErrorMessage('" + litErrorMsg.Text + "','" + Resources.Messages.Information + "','" +
                                        Page.ResolveClientUrl(Resources.PageURL.Invoicing) + "');", true);
                                }
                                else
                                {
                                    ...
                                    {
                                        litErrorMsg.Text = Resources.PageNameRes.Invoice + " " + Resources.Messages.EditUsedByAnotherUser;
                                        ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowErrorMsg", "ShowErrorMessage('" + litErrorMsg.Text
                                        + "','" + Resources.ErpRes.Information + "');", true);
                                    }
                                    else if (result == (int)DbSaveStatus.CODEEXIST)
                                    {
                                        litErrorMsg.Text = Resources.PageNameRes.Invoice + " " + Resources.Messages.Itemsalreadyaddedbyanotheruser;
                                        ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowErrorMsg", "ShowErrorMessage('" + litErrorMsg.Text
                                        + "','" + Resources.ErpRes.Information + "','" + "');", true);
                                    }
                                    else if (result == (int)DbSaveStatus.REFNOEXIST)
                                    {
                                        litErrorMsg.Text = Resources.PageNameRes.Invoice + " " + GetLocalResourceObject("RefNoExist").ToString();
                                        ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowErrorMsg", "ShowErrorMessage('" + litErrorMsg.Text
                                        + "','" + Resources.ErpRes.Information + "','" + "');", true);
                                    }
                                    else
                                    {
                                        litErrorMsg.Text = Resources.Messages.ActionFailedPleaseTryAgain;
                                        litErrorMsg.Text = string.Format(litErrorMsg.Text, Resources.PageNameRes.Invoice);
                                        ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowErrorMsg", "ShowErrorMessage('" + litErrorMsg.Text
                                            + "','" + Resources.ErpRes.Information + "');", true);
                                    }
                                }
                    ...
                    case ActionsEnum.DELETE:
                        if (CurrPK > 0)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    litErrorMsg.Text = Resources.PageNameRes.Invoice + " " + Resources.Messages.UsedInAnotherPlace;
                                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowErrorMsg", "ShowErrorMessage('" + CommonFunctions.FormatErrorMessage(litErrorMsg.Text)
                                    + "','" + Resources.ErpRes.Information + "');", true);
                                }
                                else if (result == (int)DbDeleteStatus.DELETECONCURRENCY)
                                {
                                    litErrorMsg.Text = Resources.PageNameRes.Invoice + " " + Resources.Messages.AlreadyDeleted;
                                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowErrorMsg", "ShowErrorMessage('" + CommonFunctions.FormatErrorMessage(litErrorMsg.Text)
                                        + "','" + Resources.ErpRes.Information + "');", true);
                                    EntryStatus = EntryStatus.LISTMODE;
                                    ResetForm(ControlsEnum.MISCLIST);
                                ...
                                if (hdfInvTypeText.Value != string.Empty)
                                {
                                    if (hdfInvTypeText.Value.ToString().ToLower() == "domestic")
                                    {
                                        ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "Openwindow", "OpenPDF('" + Page.ResolveClientUrl(Resources.PageURL.ReportUrl + "?ID=" +
                                            ((HiddenField)grdrow.FindControl("hdfInvoiceID")).Value + "&APPTYPE=" + ApplicationType.SI + "&APPSUBTYPE=1") + "');", true);
                                    }
                                    else if (hdfInvTypeText.Value.ToString().ToLower() == "export")
                                    {
                                        ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "Openwindow", "OpenPDF('" + Page.ResolveClientUrl(Resources.PageURL.ReportUrl + "?ID=" +
                                            ((HiddenField)grdrow.FindControl("hdfInvoiceID")).Value + "&APPTYPE=" + ApplicationType.SI + "&APPSUBTYPE=2") + "');", true);
                                    }
                                    else if (hdfInvTypeText.Value.ToString().ToLower() == "proforma")
                                    {
                                        ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "Openwindow", "OpenPDF('" + Page.ResolveClientUrl(Resources.PageURL.ReportUrl + "?ID=" +
                                            ((HiddenField)grdrow.FindControl("hdfInvoiceID")).Value + "&APPTYPE=" + ApplicationType.SI + "&APPSUBTYPE=3") + "');", true);
                                    }
                                }
                    ...
                    case ActionsEnum.DOPRINT:
                        foreach (GridViewRow grdrow in grdMiscList.Rows)
                        {
                            ...
                            {
                                ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "Openwindow", "OpenPDF('" + Page.ResolveClientUrl(Resources.PageURL.ReportUrl + "?ID=" +
                                    ((HiddenField)grdrow.FindControl("hdfInvoiceID")).Value + "&APPTYPE=" + ApplicationType.SI + "&APPSUBTYPE=21") + "');", true);
                    ...
                    case ActionsEnum.ACRECEIVABLE:
                        ...
                        Response.Redirect(Resources.PageURL.AccountReceivable);
                        break;
                    case ActionsEnum.CRDRNOTE:
                        Response.Redirect(Resources.PageURL.DrCrNoteSales);
                        break;
