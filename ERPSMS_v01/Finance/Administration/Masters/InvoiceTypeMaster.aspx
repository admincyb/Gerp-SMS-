<%@ Page Title="<%$ Resources:Captions,Title_InvoiceTypeMaster%>" Language="C#" MasterPageFile="~/ERPSMS_2.Master"
    AutoEventWireup="true" CodeBehind="InvoiceTypeMaster.aspx.cs" Inherits="ERPSMS_v01.Finance.Administration.Masters.InvoiceTypeMaster"
    ...
    <asp:UpdatePanel ID="aupdpnlTaskHome" runat="server">
        <ContentTemplate>
                                ...
                                <ul runat="server" id="pnlEntry" style="display: none">
                                    <li>
                                        <asp:Button runat="server" ID="btnSave" SkinID="btnInner-Save" Text="<%$Resources:Controls,Save%>"
                                            ...
                                            OnClick="ActionHandler" ValidationGroup="InvoiceTypeSave" OnClientClick="javascript:ValidateNow('InvoiceTypeSave')"
                                            TabIndex="35" />
                                    </li>
                ...
                <asp:Table runat="server" ID="tblGstClassification" CssClass="tablelayout asptbllinks">
                    <asp:TableRow ID="PageAction_List" runat="server" Style="display: none;">
                        <asp:TableCell>
                            ...
                            <div class="gridwrap">
                                <asp:GridView runat="server" ID="grdInvoiceTypeList" Width="100%" PageSize="<%$ resources:PageSize%>"
                                    ...
                                    <Columns>
                                        <asp:TemplateField>
                                            <ItemTemplate>
                                                ...
                                                <asp:HiddenField runat="server" ID="hdfGstCfnPk" Value='<%# Eval("FTM_PK") %>' />
                                            </ItemTemplate>
                                            <ItemStyle HorizontalAlign="Center" Width="0.5%" />
                                        </asp:TemplateField>
                                        <asp:TemplateField HeaderText="<%$ resources:Code %>">
                                            <ItemTemplate>
                                                <asp:Label ID="lblgrdGstClassCode" runat="server" Text='<%# ERP.Utilities.CommonFunctions.GetShortString(ERP.Utilities.CommonFunctions.GetEncodedString(Eval("FTM_CODE")),20) %>'
                                                    ToolTip='<%# System.Web.HttpUtility.HtmlDecode(Convert.ToString(Eval("FTM_CODE")))%>'></asp:Label>
                                            </ItemTemplate>
                                            <ItemStyle Width="5%" />
                                        </asp:TemplateField>
                                        <asp:TemplateField HeaderText="<%$ resources:Name %>">
                                            <ItemTemplate>
                                                <asp:Label ID="lblgrdGstClassName" runat="server" Text='<%# ERP.Utilities.CommonFunctions.GetShortString(ERP.Utilities.CommonFunctions.GetEncodedString(Eval("FTM_NAME")),20) %>'
                                                    ToolTip='<%# System.Web.HttpUtility.HtmlDecode(Convert.ToString(Eval("FTM_NAME")))%>'></asp:Label>
                                            </ItemTemplate>
                                            <ItemStyle Width="9%" />
                                        </asp:TemplateField>
                                        <asp:TemplateField HeaderText="<%$ resources:InvoiceType %>">
                                            <ItemTemplate>
                                                <asp:Label ID="lblgrdGstClassTaxability" runat="server" Text='<%# ERP.Utilities.CommonFunctions.GetShortString(ERP.Utilities.CommonFunctions.GetEncodedString(Eval("FTM_INVOICE_TYPE_TEXT")),20) %>'
                                                    ToolTip='<%# System.Web.HttpUtility.HtmlDecode(Convert.ToString(Eval("FTM_INVOICE_TYPE_TEXT")))%>'></asp:Label>
                                            </ItemTemplate>
                                            <ItemStyle Width="6%" />
                                        </asp:TemplateField>
                                        <asp:TemplateField HeaderText="<%$ resources:Description %>">
                                            <ItemTemplate>
                                                <asp:Label ID="lblgrdGstClassDescription" runat="server" Text='<%# ERP.Utilities.CommonFunctions.GetShortString(ERP.Utilities.CommonFunctions.GetEncodedString(Eval("FTM_DESC")),80) %>'
                                                    ToolTip='<%# System.Web.HttpUtility.HtmlDecode(Convert.ToString(Eval("FTM_DESC")))%>'></asp:Label>
                                            </ItemTemplate>
                                            <ItemStyle Width="20%" />
                                        </asp:TemplateField>
                                        <asp:TemplateField HeaderText="<%$ resources:Active%>">
                                            <ItemTemplate>
                                                <image id="imgStatus" alt="" title='<%# (Eval("FTM_ACTIVE")).ToString()=="1"? Resources.ErpRes.Active :Resources.ErpRes.InActive %>'
                                                    class='<%# (Eval("FTM_ACTIVE")).ToString()=="1"?"active" :"inactive"%>'></image>
                                            </ItemTemplate>
                        ...
                        <asp:TableCell>
                            <table class="table-devide">
                                <tr>
                                    <td>
                                        <div class="div2col-S">
                                            <asp:Label runat="server" ID="lblInvTypeName" Text="<%$ resources:NameStar%>" AssociatedControlID="txtInvTypeName"></asp:Label>
                                            <asp:TextBox ID="txtInvTypeName" runat="server" CssClass="input-w45per" TabIndex="1"></asp:TextBox>
                                            <asp:RequiredFieldValidator ID="rfvName" runat="server" Text="*" ErrorMessage="<%$ resources:Err_EnterName%>"
                                                CssClass="star" ControlToValidate="txtInvTypeName" ValidationGroup="InvoiceTypeSave">
                                            </asp:RequiredFieldValidator>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="div2col-S">
                                            <asp:Label runat="server" ID="lblInvTypeCode" Text="<%$ resources:Code%>" AssociatedControlID="txtInvTypeCode"></asp:Label>
                                            <asp:TextBox ID="txtInvTypeCode" runat="server" CssClass="input-medium" TabIndex="1"></asp:TextBox>
                                            <%-- <asp:RequiredFieldValidator ID="rfvCode" runat="server" Text="*" ErrorMessage="<%$ resources:Err_EnterCode%>"
                                                CssClass="star" ControlToValidate="txtInvTypeCode" ValidationGroup="InvoiceTypeSave">
                                            </asp:RequiredFieldValidator>--%>
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                        <div class="div2col-S">
                                            <asp:Label runat="server" ID="lblInvoiceGroup" Text="<%$ resources:Group%>" AssociatedControlID="ddlInvoiceGroup"></asp:Label>
                                            <asp:DropDownList runat="server" ID="ddlInvoiceGroup" CssClass="lbl-31-1perc" TabIndex="2"
                                                OnSelectedIndexChanged="ActionHandler" AutoPostBack="true">
                                            </asp:DropDownList>
                                            <asp:RequiredFieldValidator ID="rfvInvoiceGroup" runat="server" Text="*" ErrorMessage="<%$ resources:Err_Group%>"
                                                CssClass="star" ControlToValidate="ddlInvoiceGroup" ValidationGroup="InvoiceTypeSave"
                                                InitialValue="-1">
                                            </asp:RequiredFieldValidator>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="div2col-S">
                                            <asp:Label runat="server" ID="lblInvoiceType" Text="<%$ resources:Type%>" AssociatedControlID="ddlInvoiceType"></asp:Label>
                                            <asp:DropDownList runat="server" ID="ddlInvoiceType" CssClass="lbl-31-1perc" TabIndex="2">
                                            </asp:DropDownList>
                                            <asp:RequiredFieldValidator ID="rfvInvoiceType" runat="server" Text="*" ErrorMessage="<%$ resources:Err_Type%>"
                                                CssClass="star" ControlToValidate="ddlInvoiceType" ValidationGroup="InvoiceTypeSave"
                                                InitialValue="-1">
                                            </asp:RequiredFieldValidator>
                                        </div>
                                    </td>
                                    ...
                                    <td colspan="2">
                                        <div class="divcol-S">
                                            <asp:Label runat="server" ID="lblInvTypeDescription" Text="<%$ resources:Description%>"
                                                AssociatedControlID="txtInvTypeDescription"></asp:Label>
                                            <asp:TextBox ID="txtInvTypeDescription" runat="server" TabIndex="3" MaxLength="450"
            ...
            <div id="diverror" style="display: none">
                <asp:ValidationSummary ID="vsSave" ValidationGroup="InvoiceTypeSave" runat="server" />
