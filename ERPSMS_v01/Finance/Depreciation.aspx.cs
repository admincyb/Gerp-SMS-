...
{
    public partial class Depreciation : ERP.Store.UI.WorkFlowBasePage
    {
        ...
        /// </summary>
        private DepreciationBO SelectedDepreciation
        {
            get
            {
                return (DepreciationBO)ViewState[ERP.Utilities.SessionStrings.SelectedDepreciation];
            }
            set
            {
                ViewState[ERP.Utilities.SessionStrings.SelectedDepreciation] = value;
            }
        }
        ...
        private int AssetTypePK
        {
            get
            {
                return Convert.ToInt32(this.ViewState["AssetTypePK"]);
            }
            set
            {
                this.ViewState["AssetTypePK"] = value;
            }
        }
            ...
            try
            {
                ...
                else if (((GridView)sender).ID == "grdAssetList")
                {
                    if (e.Row.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        string IsAlreadyExist = Convert.ToString(DataBinder.Eval(e.Row.DataItem, "IS_ENTRY_VALIDATE"));
                        ...
                        if (IsAlreadyExist == "1")
                        {
                            e.Row.Attributes["style"] = "background-color: " + GetLocalResourceObject("rowGreen").ToString();
                    ...
                    {
                        lblCountAssets.Text = "Asset Count :" + string.Empty;
                    }

                    if (prevasset)
                    {
                        grdAssetList.Columns[0].Visible = false;
                        btnAddToList.Visible = false;
                    }
                    else
                    {
                        grdAssetList.Columns[0].Visible = true;
                ...
                else if (((GridView)sender).ID == "grdSelectedAssets")
                {
                    if (e.Row.RowType == DataControlRowType.Footer)
                    {
                        ...
                        Label lblTotalAssetCostAmtFooter = (Label)e.Row.FindControl("lblTotalAssetCostAmtFooter");
                        if (lblTotalAssetDepreAmtFooter != null)
                        {
                            if (this.SelectedDepreciation.Details != null && this.SelectedDepreciation.Details.Any())
                            {
                                string _curFormat = "N" + hdfDecimalDigits.Value.Trim();
                                lblTotalAssetCostAmtFooter.Text = this.SelectedDepreciation.Details.Sum(x => x.amiCostLand).ToString(_curFormat);
                                lblTotalAssetCostAmtFooter.ToolTip = this.SelectedDepreciation.Details.Sum(x => x.amiCostLand).ToString(_curFormat);
                            ...
                            else
                            {
                                ...
                                lblTotalAssetCostAmtFooter.Text = string.Empty;
                                lblTotalAssetCostAmtFooter.ToolTip = null;
                            }
                        }
                    }
        ...
        private void PageActionHandler()
        {
            ...
            try
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    txtFromDate.Text = GetGlobalResourceObject("ConfigurationsRes", "IsDepreSearchFromPreviousMonth").ToString() == "1" ?
                        DateTime.Now.AddMonths(-1).StartDateOfTheMonth().ToString(Resources.Constants.DateFormatShort) :
                        DateTime.Now.StartDateOfTheYear().ToString(Resources.Constants.DateFormatShort);
                    txtToDate.Text = GetGlobalResourceObject("ConfigurationsRes", "IsDepreSearchToCurrntMonth").ToString() == "1" ? DateTime.Now.EndDateOfTheMonth().ToString(Resources.Constants.DateFormatShort) :
                        DateTime.Now.EndDateOfTheYear().ToString(Resources.Constants.DateFormatShort);
            ...
            try
            {
                ...
                switch (commonActions)
                {
                    ...
                    case ActionsEnum.LIST:
                        ...
                        this.SelectedDepreciation = null;
                        this.CurrPK = 0;
                    ...
                    case ActionsEnum.NEW:
                        ...
                        this.CurrPK = 0;
                        this.SelectedDepreciation = new DepreciationBO();
                        this.SelectedDepreciation.FDH_PK = this.CurrPK;
                    ...
                    case ActionsEnum.PRINT:
                        ScriptManager.RegisterStartupScript(this.Page,
                            typeof(Page),
                            "Openwindow",
                            "OpenPDF('" + Page.ResolveClientUrl(Resources.PageURL.ReportUrl + "?ID=" + CurrPK.ToString() + "&APPTYPE=" + ApplicationType.DPR) + "');",
                            true);
                        //GetFieldValues(ControlEnums.SELECTEDDEPRECIATION);
                        //SetFieldValues(ControlEnums.SELECTEDDEPRECIATION);
                    ...
                    case ActionsEnum.GETLIST:
                        ...
                        if (this.SelectedDepreciation.Details.Count != 0)
                        {
                            ...
                            if (GetGlobalResourceObject("ConfigurationsRes", "DisableAssetType").ToString() == "1")
                            {
                                ddlAssetType.SelectedIndex = ddlAssetType.Items.IndexOf(ddlAssetType.Items.FindByValue(AssetTypePK.ToString()));
                                // ddlPlant.SelectedValue =Convert.ToString(ddlPlant.Items.IndexOf(ddlPlant.Items.FindByValue(PlantPK.ToString())));
                        ...
                        {
                            ddlPlant.Enabled = ddlAssetType.Enabled = true;
                        }
                    ...
                    case ActionsEnum.ADDTOLIST:
                        #region Validation - Depreciation already exist for the period
                        List<string> AssetsExist = new List<string>();
                        string AssetCodes = string.Empty;
                        foreach (GridViewRow row in grdAssetList.Rows)
                        {
                            HiddenField hdfIsAlreadyAdded = (HiddenField)row.FindControl("hdfIsAlreadyExist");
                            Label lblAssetCode = (Label)row.FindControl("lblAssetCode");
                            CheckBox chk = (CheckBox)row.FindControl("chkSelect");
                            if (chk != null && chk.Checked && hdfIsAlreadyAdded.Value == "1")
                            {
                                AssetsExist.Add(lblAssetCode.Text);
                            }
                            AssetCodes = string.Join(", ", AssetsExist.ToArray());
                        }
                        if (AssetsExist.Any())
                        {
                            ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowPop1", "ShowContainerDiv('[id$=divPckAssetPopupContainer]','" + Resources.Captions.SelectAssetsForDepreciation + "','900','500');", true);
                            litErrorMsg.Text = string.Format(GetLocalResourceObject("Msg_AlreadyExist").ToString(), AssetCodes);
                        ...
                        if (assetAddedToList)
                        {
                            ...
                            AssetTypePK = Convert.ToInt32(ddlAssetType.SelectedValue);
                    ...
                    case ActionsEnum.SAVE:
                        ...
                        else if (this.SelectedDepreciation.Details == null || this.SelectedDepreciation.Details.Count == 0)
                        {
                        ...
                        {
                            //DepreciationBO depreciationTran = (DepreciationBO)SetUIValuesToObject(ControlEnums.SELECTEDDEPRECIATION);
                            //string depreNo = string.Empty;
                            //result = DepreciationBL.Save(depreciationTran, out depreNo);

                            DepreciationBO depreciationTran = (DepreciationBO)SetUIValuesToObject(ControlEnums.SELECTEDDEPRECIATION);
                            depreciationTran.WKF_FLAG = Convert.ToInt32(WorkflowTransactionFlag.SAVE);
                            depreciationTran.UserPk = currentUser.PKUser;
                            // save Process Control inspection details
                            string xmlDoc = CommonFunctions.XmlSerialize<DepreciationBO>(depreciationTran);
                            result = DepreciationBL.SaveDepreciationWkf(xmlDoc, out retRefID, out transNo);
                            if (result > 0)
                            {
                                litErrorMsg.Text = Resources.ErrorMessages.Msg_Save_Success;
                                litErrorMsg.Text = string.Format(litErrorMsg.Text, Resources.PageNameRes.DepreciationInvoice);
                            ...
                            else
                            {
                                ...
                                {
                                    litErrorMsg.Text = Resources.PageNameRes.DepreciationInvoice + " " + Resources.Messages.EditUsedByAnotherUser;
                                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowErrorMsg", "ClosePopup();ShowErrorMessage('" + litErrorMsg.Text
                                    + "','" + Resources.ErpRes.Information + "');", true);
                                }
                                else if (result == (int)DbDeleteStatus.DELETECONCURRENCY)
                                {
                                    litErrorMsg.Text = Resources.PageNameRes.DepreciationInvoice + " " + Resources.Messages.AlreadyDeleted;
                                ...
                                {
                                    litErrorMsg.Text = Resources.PageNameRes.DepreciationInvoice + " " + Resources.Messages.Itemsalreadyaddedbyanotheruser;
                                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowErrorMsg", "ClosePopup();ShowErrorMessage('" + litErrorMsg.Text
                                    + "','" + Resources.ErpRes.Information + "','" + "');", true);
                                }
                                else if (result == (int)DbSaveStatus.AMOUNTEXCEEDED)
                                {
                                    litErrorMsg.Text = Resources.PageNameRes.Asset + " " + GetLocalResourceObject("DepreciationAmountExceeded").ToString();
                                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowErrorMsg", "ClosePopup();ShowErrorMessage('" + litErrorMsg.Text
                                    + "','" + Resources.ErpRes.Information + "','" + "');", true);
                                }
                                else
                    ...
                    case ActionsEnum.WRKFSUBMIT:
                        ...
                        else//valid
                        {
                            ...
                            //        if (depreciationTran != null && depreciationTran.Details != null)
                            //        {
                            //            string depreNo = string.Empty;
                            //            result = DepreciationBL.Save(depreciationTran, out depreNo);
                            ...
                            //                if (string.IsNullOrEmpty(lblDepreciationNo.Text.Trim())
                            //                            || lblDepreciationNo.Text.Trim() == Resources.Messages.DocGenerationNew)
                            //                {
                            //                    CurrPK = ucrWrkf.ApplicationID;
                            //                    GetFieldValues(ControlEnums.SELECTEDDEPRECIATION);
                            //                    lblDepreciationNo.Text = depreciationTran.FDH_NO;
                            //                }
                            //            }
                            //            else
                            //            {
                            ...
                            //                else if (result == (int)DbSaveStatus.AMOUNTEXCEEDED)
                            //                {
                            //                    litErrorMsg.Text = Resources.PageNameRes.DepreciationInvoice + " " + GetLocalResourceObject("DepreciationAmountExceeded").ToString();
                            //                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowErrorMsg", "ShowErrorMessage('" + litErrorMsg.Text
                            //                    + "','" + Resources.ErpRes.Information + "','" + "');", true);
                            //                }
                            //                else
                            ...
                            //{
                            //    if (BusinessLogic.CommonManagement.CommonBL.ValidationForCancellation(CurrPK, ApplicationType.DPR))
                            //    {
                            ...
                            //                CurrPK = ucrWrkf.ApplicationID;
                            //                GetFieldValues(ControlEnums.SELECTEDDEPRECIATION);
                            //                invoiceNo = this.SelectedDepreciation.FDH_NO;
                            //            }
                            //            else
                            //            {
                            //                invoiceNo = lblDepreciationNo.Text.Trim();
                            //            }

                            //            object[] args = new object[2];
                            //            args[0] = Resources.PageNameRes.DepreciationInvoice;
                            //            args[1] = invoiceNo;
                            ...
                            //            AdmTrxLogDet.ATL_APP_TYPE = ApplicationType.DPR;
                    ...
                    case ActionsEnum.DELETE:
                        if (CurrPK > 0)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    litErrorMsg.Text = Resources.PageNameRes.DepreciationInvoice + " " + Resources.Messages.UsedInAnotherPlace;
                                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowErrorMsg", "ShowErrorMessage('" + CommonFunctions.FormatErrorMessage(litErrorMsg.Text)
                                    + "','" + Resources.ErpRes.Information + "');", true);
                                }
                                else if (result == (int)DbDeleteStatus.DELETECONCURRENCY)
                    ...
                    case ActionsEnum.REMOVEITEM:
                        ...
                        if (hdf != null)
                        {
                            ...
                            else
                            {
                                ...
                                DepreciationDetails det = this.SelectedDepreciation.Details.Single(x => x.FDD_ASSET == _selectedPK);
                                if (this.SelectedDepreciation.Details.Remove(det))
                                {
                                    litErrorMsg.Text = Resources.ErrorMessages.Msg_Details_Removed_Success;
                                    litErrorMsg.Text = string.Format(litErrorMsg.Text, Resources.PageNameRes.Asset);
                                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowErrorMsg", "ShowErrorMessage('" + litErrorMsg.Text + "','" + Resources.Messages.Information + "');", true);
                    ...
                    case ActionsEnum.JOURNALIZECANCEL:
                        ...
                        break;
                    #endregion
                    #region Prev Asset Search
                    case ActionsEnum.PREVASSETSEARCH:
                        prevasset = true;
                        GetFieldValues(ControlEnums.PREVASSETLIST);
                        SetFieldValues(ControlEnums.ASSETLIST);
                        ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowPop1", "ShowContainerDiv('[id$=divPckAssetPopupContainer]','" + Resources.Captions.SelectAssetsForDepreciation + "','900','500');", true);
                        break;
                        #endregion
            ...
            try
            {
                switch (type)
                {
                    ...
                    case ControlEnums.DEPRETRANLIST:
                        ...
                        int asset_Type = Convert.ToInt32(ddlAssetTypeSearch.SelectedValue) > 0 ? Convert.ToInt32(ddlAssetTypeSearch.SelectedValue) : 0;
                        if (Type == 2)
                        {
                            PType = 2;
                        }
                        ...
                        dtPageData = DepreciationBL.GetDepreciationTranList(new BusinessObject.GridPrams()
                        {
                            SortBy = string.IsNullOrEmpty(SortBy) ? Resources.DataFieldRes.DepreTranDate : SortBy,
                            SortDirection = string.IsNullOrEmpty(SortDirection)
                                                    ? Resources.Report.SortDescending
                                                    : SortDirection,
                            ThenBy = SortBy == ThenBy || SortBy == Resources.DataFieldRes.DepreTranNo
                                                    ? string.Empty
                                                    : string.IsNullOrEmpty(ThenBy)
                                                        ? Resources.DataFieldRes.DepreTranNo
                                                        : ThenBy,
                            ThenDirection = SortBy == ThenBy || SortBy == Resources.DataFieldRes.DepreTranNo
                                                    ? string.Empty
                            ...
                            ToDate = string.IsNullOrEmpty(txtToDate.Text.Trim())
                                                    ? string.Empty
                                                    : txtToDate.Text.Trim(),
                            SearchBy = Resources.DataFieldRes.DepreTranNo,
                        ...
                        }, currentUser.PKUser, currentUser.SBUID, _pageUrl, PType, asset_Type, Plant, Status);
                        break;
                    #endregion
                    #region DEPREGET
                    case ControlEnums.DEPREGET:
                        ...
                        dtPageData = DepreciationBL.GetDepreciationTranList(new BusinessObject.GridPrams()
                        {
                            ...
                            PageNumber = 1,
                            PageSize = this.PageSize
                        }, currentUser.PKUser, currentUser.SBUID, _pgeUrl, 3, 0);
                        break;
                    #endregion
                    #region Company
                    ...
                    case ControlEnums.SELECTEDDEPRECIATION:
                        this.SelectedDepreciation = DepreciationBL.GetDepreciationDetails(this.CurrPK);
                        break;
                    #endregion
                    #region ASSETLIST
                    case ControlEnums.ASSETLIST:
                        ...
                        dtPageData = DepreciationBL.GetAssetList(new AssetFilterForDepreParameterBinder
                        {
                    ...
                    case ControlEnums.SELECTEDASSETLIST:
                        ...
                        if (this.SelectedDepreciation != null)
                        {
                            foreach (GridViewRow row in grdAssetList.Rows)
                            {
                                CheckBox chk = (CheckBox)row.FindControl("chkSelect");
                                if (chk != null && chk.Checked)
                                {
                                    ...
                                    if (hdf != null)
                                    {
                                        ...
                                        if (GetGlobalResourceObject("ConfigurationsRes", "NegativeDepreciationAmtValidation").ToString() == "1")
                                        {
                                            if (!this.SelectedDepreciation.Details.Any(x => x.FDD_ASSET == assetPk))
                                            {
                                                this.SelectedDepreciation.Details.Add(new DepreciationDetails
                                                {
                                                    FDD_PK = detailPk, // New Entry
                                                    FDD_ASSET = assetPk,
                                                    ...
                                                    FDD_ACTIVE = active,
                                                    FDD_AMOUNT = depreAmt,
                                                    FDD_HDR = this.SelectedDepreciation.FDH_PK,
                                        ...
                                        {
                                            if (!this.SelectedDepreciation.Details.Any(x => x.FDD_ASSET == assetPk) && depreAmt > 0)
                                            {
                                                this.SelectedDepreciation.Details.Add(new DepreciationDetails
                                                {
                                                    FDD_PK = detailPk, // New Entry
                    ...
                    case ControlEnums.GETEXPENSEPKBYJOURNALPK:
                        ...
                        break;
                    #endregion
                    #region PREVASSETLIST
                    case ControlEnums.PREVASSETLIST:
                        assetCategory = Convert.ToInt32(CommonConstants.SELECTVAL); // Asset Category is Removed From UI
                        AssetCode = txtPopUpAssetCode.Text.Trim();
                        AssetName = txtPopUpAssetName.Text.Trim();
                        assetType = Convert.ToInt32(ddlAssetType.SelectedValue);
                        plant = Convert.ToInt32(ddlPlant.SelectedValue);
                        if (Type == 2)
                        {
                            PType = 2;
                        }
                        ...
                        dtPageData = DepreciationBL.GetPrevAssetList(new AssetFilterForDepreParameterBinder
                        {
                            AssetCategory = assetCategory,
                            AssetType = assetType,
                            AssetCode = AssetCode,
                ...
                switch (controlType)
                {
                    ...
                    case ControlEnums.SELECTEDDEPRECIATION:
                        if (this.SelectedDepreciation == null)
                        {
                            ...
                            throw new ApplicationException(string.Format(errMsgTempale, Resources.PageNameRes.DepreciationInvoice));
                        }
            ...
            try
            {
                switch (controlType)
                {
                    ...
                    case ControlEnums.SELECTEDDEPRECIATION:
                        ...
                        lblDepreciationNo.Text = this.SelectedDepreciation.FDH_NO == string.Empty ? "[NEW]" : this.SelectedDepreciation.FDH_NO;
                        txtDepreDate.Text = this.SelectedDepreciation.FDH_DATE.ToString(Resources.Constants.DateFormatShort);
                        txtFromDateEntry.Text = this.SelectedDepreciation.FDH_FROM.ToString(Resources.Constants.DateFormatShort);
                        txtToDateEntry.Text = this.SelectedDepreciation.FDH_TO.ToString(Resources.Constants.DateFormatShort);
                        txtDepreMonth.Text = this.SelectedDepreciation.FDH_MONTH_YEAR.ToString(Resources.Constants.DateFormatMonthYear);
                        txtDescription.Text = this.SelectedDepreciation.FDH_DESC.IsNullOrEmptyOrWhitespace()
                                                ? String.Empty
                                                : this.SelectedDepreciation.FDH_DESC.Trim();
                        this.LastModifiedTime = this.SelectedDepreciation.LAST_MOD_DT;
                        Approved = Convert.ToInt32(this.SelectedDepreciation.FDH_STATUS);
                        Posted = Convert.ToBoolean(this.SelectedDepreciation.FDH_HAS_JRNL_ENTRY);
                        txtToDateEntry.Enabled = false;
                        txtFromDateEntry.Enabled = false;
                        txtDepreMonth.Enabled = false;
                        ModifiedDatePnl.Visible = true;
            ...
            try
            {
                switch (controlType)
                {
                    ...
                    case ControlEnums.SELECTEDDEPRECIATION:
                        ...
                        this.SelectedDepreciation.FDH_NO = lblDepreciationNo.Text == "[NEW]" ? string.Empty : lblDepreciationNo.Text;
                        this.SelectedDepreciation.FDH_DATE = Convert.ToDateTime(txtDepreDate.Text.Trim());
                        this.SelectedDepreciation.FDH_FROM = Convert.ToDateTime(txtFromDateEntry.Text.Trim());
                        this.SelectedDepreciation.FDH_TO = Convert.ToDateTime(txtToDateEntry.Text.Trim());
                        this.SelectedDepreciation.FDH_MONTH_YEAR = Convert.ToDateTime(txtDepreMonth.Text.Trim());
                        this.SelectedDepreciation.FDH_DESC = HttpUtility.HtmlEncode(txtDescription.Text.Trim());

                        if (this.CurrPK == 0)
                            this.SelectedDepreciation.LAST_MOD_DT = DateTime.Now;
                        else
                            this.SelectedDepreciation.LAST_MOD_DT = this.LastModifiedTime;

                        if (Type == 2)
                            this.SelectedDepreciation.FDH_TYPE = 2;
                        else
                            this.SelectedDepreciation.FDH_TYPE = 1;

                        this.SelectedDepreciation.FDH_DEPT = currentUser.CurrentDeptPK;
                        this.SelectedDepreciation.BizUnit = currentUser.SBUID;
                        this.SelectedDepreciation.Active = ((int)DbActiveStatus.ACTIVE);
                        this.SelectedDepreciation.UserPk = currentUser.PKUser;

                        this.SelectedDepreciation.FDH_TRX_CURR = this.SelectedDepreciation.FDH_BASE_CURR = currentUser.BaseCurrency <= 0
                                                ? 1 : currentUser.BaseCurrency;
                        this.SelectedDepreciation.FDH_COMPANY = ddlPlant.SelectedValue;

                        SetDepreciationAmount();

                        this.SelectedDepreciation.FDH_AMOUNT_BC = this.SelectedDepreciation.FDH_AMOUNT_TC = SelectedDepreciation
                                                .Details
                                                .Sum(x => x.FDD_AMOUNT);
                        this.SelectedDepreciation.FDH_EXCHG_RATE = 1;

                        #region Validate Depreciation Amount
                        if (GetGlobalResourceObject("ConfigurationsRes", "NegativeDepreciationAmtValidation").ToString() != "1")
                        {
                            if (this.SelectedDepreciation.Details.Any(x => x.FDD_AMOUNT <= 0))
                            {
                        ...
                        if (this.SelectedDepreciation.Details.GroupBy(x => x.FDD_ASSET).Any(g => g.Count() > 1))
                        {
                            throw new ApplicationException("Duplicate Items found");
                        }
                        #endregion

                        retObject = this.SelectedDepreciation;
                        break;
                    #endregion
                    #region JOURNALIZE
                    case ControlEnums.JOURNALIZE:
                        ...
                        if (bIsChecked)
                        {
                            if (Approved == 2)
                            {
                                ...
                                Session[ERP.Utilities.SessionStrings.TransactionNo] = this.SelectedDepreciation.FDH_NO;
                                Session[ERP.Utilities.SessionStrings.TransactionDate] = this.SelectedDepreciation.FDH_DATE;
                                Session[ERP.Utilities.SessionStrings.TransactionCurrency] = this.SelectedDepreciation.FDH_TRX_CURR;
                                Session[ERP.Utilities.SessionStrings.JournalType] = ApplicationType.DPRJ;
        ...
        private void BindGrid(ControlEnums controlType)
        {
            switch (controlType)
            {
                ...
                case ControlEnums.SELECTEDASSETLIST:
                    if (this.SelectedDepreciation != null)
                    {
                        grdSelectedAssets.DataSource = this.SelectedDepreciation.Details;
                    }
            ...
            switch (controlType)
            {
                ...
                case ControlEnums.ASSETTYPE:
                    if (dtPageData != null && dtPageData.Rows.Count > 0)
                    {
                        ...
                        ddlAssetTypeSearch.Items.Clear();
                        ddlAssetTypeSearch.DataSource = dtPageData;
                        ddlAssetTypeSearch.DataTextField = Resources.DataFieldRes.atpName;
                        ddlAssetTypeSearch.DataValueField = Resources.DataFieldRes.atpPK;
                        ddlAssetTypeSearch.DataBind();
                        ddlAssetTypeSearch.Items.HtmlDecode();
                        //ddlAssetType.Items.Insert(0, new ListItem { Text = "Select", Value = CommonConstants.SELECTVAL });
                        //ddlAssetTypeSearch.SelectedIndex = ddlAssetTypeSearch.Items.IndexOf(ddlAssetTypeSearch.Items.FindByValue(dtPageData.Rows[0][Resources.DataFieldRes.atpPK].ToString()));

                    }

                    ddlAssetTypeSearch.Items.Insert(0, new ListItem(Resources.ErpRes.Select, CommonConstants.SELECTVAL)); break;
                case ControlEnums.PLANT:
        ...
        private void SaveTransaction(DepreciationBO objDepreciation, int workflowFlag, object sender)
        {
            try
            {
                int retRefID;
                ...
                if (objDepreciation == null)
                    objDepreciation = new DepreciationBO();
                ...
                #endregion
                objDepreciation.APT_CODE = ApplicationType.DPR;

                //objDepreciation.CWIPDate = Convert.ToDateTime(txtCWIPDate.Text);
                string xmlDoc = CommonFunctions.XmlSerialize<DepreciationBO>(objDepreciation);
                result = DepreciationBL.SaveDepreciationWkf(xmlDoc, out retRefID, out transNo);
                if (result.HasValue && result.Value > 0) // Success !  redirect to listing page
                {
                    CurrPK = (int)result;
                    if (hdfIsSaveSubmit.Value == CommonConstants.SELECTVAL)
                        litErrorMsg.Text = string.Format(Resources.Messages.Msg_Cancelled_Success, Resources.PageNameRes.DepreciationInvoice);
                    else
                        litErrorMsg.Text = string.Format(GetLocalResourceObject("Msg_Submit_Success").ToString(), Resources.PageNameRes.DepreciationInvoice, transNo);
                ...
                else
                {
                    ...
                    {
                        litErrorMsg.Text = GetLocalResourceObject("DeprecsPrevValid").ToString();
                        ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowErrorMsg", "ClosePopup();ShowErrorMessage('" + litErrorMsg.Text
                        + "','" + Resources.ErpRes.Information + "','" + "');", true);
                    }
                    else
        ...
        {
            if (this.SelectedDepreciation != null & this.SelectedDepreciation.Details.Any())
            {
                foreach (GridViewRow row in grdSelectedAssets.Rows)
                {
                    ...
                    if (hdf != null && hdf.Value.Trim() != string.Empty)
                    {
                        ...
                        this.SelectedDepreciation.Details.SingleOrDefault(x => x.FDD_ASSET == assetPk).FDD_AMOUNT = amt;
                    }
                }
            }
        }
