...
{
    public sealed class PDCVoucherPaymentReportBuilder : ReportDataBuilder
    {
        #region Properties
        PDCVoucherPayment reportData;
        #endregion

        #region Constructor
        public PDCVoucherPaymentReportBuilder(string rptType)
            : base(rptType)
        {
            reportData = new PDCVoucherPayment();
        }
        #endregion
        ...
        private void SetReportData(PaymentVoucherReportParameter parameter)
        {
            ...
            reportData.Amount = additionalReportDataSource.FTR_DR_AMT_TC.Value.ToString(reportData.CurrencyFormat);  //parameter.reportDataSourceList.Sum(x => x.PVM_PAID_AMOUNT_TC).ToString(reportData.CurrencyFormat);
            ...
            foreach (SPFIN_TRX_VOUCHER_RPT_Result subItem in parameter.additionalReportDataSourceList)
            {
                reportData.ItemDetails.Add(new PaymentVoucherItem
                {
                    ...
                    GLName = HttpUtility.HtmlDecode(subItem.FTR_ACCOUNT_NAME)
                             + (CommonFunctions.IsNullOrEmptyOrWhitespace(subItem.FTR_NARRATION)
                                    ? CommonConstants.HTML_SPACE
                                    : HttpUtility.HtmlDecode(subItem.FTR_NARRATION)),
                    ExRate = subItem.FTR_EXCHG_RATE.HasValue && subItem.FTR_EXCHG_RATE.Value > 0
            ...
            decimal debitTotal = parameter.additionalReportDataSourceList.Sum(x => x.FTR_DR_AMT_BC ?? 0);
            reportData.AmountInWords = new NumberToWordsConvertorFactory(reportData.BaseCurrency)
                                           .GetNumberToWordsConvertor()
                                           .ConvertNumberToWords(debitTotal.ToString(reportData.CurrencyFormat).Replace(",", string.Empty))
                                           .Replace("Paise",additionalReportDataSource.FTH_BASE_CURR_FRACTION);
            ...
            reportData.Narration = HttpUtility.HtmlDecode(reportDataSource.FTH_NARRATION ?? CommonConstants.HTML_SPACE);
           ...
           /* reportData.PreparedBy = reportDataSource.FTH_TASK1_BY_TEXT ?? CommonConstants.HTML_SPACE;
            reportData.ApprovedBy = reportDataSource.FTH_TASK2_BY_TEXT ?? CommonConstants.HTML_SPACE;
            reportData.AutherizedBy = reportDataSource.FTH_TASK3_BY_TEXT ?? CommonConstants.HTML_SPACE;
            reportData.ReceivedBy = reportDataSource.FTH_TASK4_BY_TEXT ?? CommonConstants.HTML_SPACE;*/
            ...
            //                            : CommonConstants.HTML_SPACE;
            //reportData.ApprovedByDate = reportDataSource.FTH_TASK2_DT_TEXT.HasValue
            //                            ? reportDataSource.FTH_TASK2_DT_TEXT.Value.ToString(reportData.DateFormat)
            //                            : CommonConstants.HTML_SPACE;
            //reportData.AutherizedByDate = reportDataSource.FTH_TASK3_DT_TEXT.HasValue
            //                            ? reportDataSource.FTH_TASK3_DT_TEXT.Value.ToString(reportData.DateFormat)
            //                            : CommonConstants.HTML_SPACE;
            //reportData.ReceivedByDate = reportDataSource.FTH_TASK4_DT_TEXT.HasValue
        ...
        private string buildReportBody()
        {
            ...
            string reoportBody = string.Format(
                                       ...
                                       reportData.BillItemBaseCurrency,  // 15  
                                       _billItemRowData,                 // 16
                                       reportData.BaseCurrency,          // 17
                                       _rowData,                         // 18
                                       reportData.AmountBeforeVat,       // 19
                                       reportData.Vat,                   // 20
                                       reportData.Total,                 // 21
                                       reportData.WithHoldingTax,        // 22
                                       reportData.NetPayment,            // 23
                                       ...
                                       reportData.ApprovedBy,            // 31
                                       reportData.ApprovedByDate,        // 32
                                       reportData.AutherizedBy,          // 33
                                       reportData.AutherizedByDate,      // 34
                                       reportData.ReceivedBy,            // 35
                                       reportData.ReceivedByDate,        // 36
                                       reportData.FooterText             // 37
                                );
            #region Return Value
