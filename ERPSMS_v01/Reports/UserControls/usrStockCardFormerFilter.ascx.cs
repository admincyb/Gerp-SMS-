...
{
    public partial class usrStockCardFormerFilter : MISFilterBase
    {
        ...
        #region Properties
        private bool usrStockCardFormerFilter_IsPostBack
        {
            get
            {
                return this.ViewState["usrStockCardFormerFilter_IsPostBack"] == null ? false : (bool)this.ViewState["usrStockCardFormerFilter_IsPostBack"];
            }
            set
            {
                this.ViewState["usrStockCardFormerFilter_IsPostBack"] = value;
            }
        }
        #endregion
        #endregion
        ...
        public override ReportParameters GetReportParameters()
        {
            try
            {
                ...
                tempReportParams.FromDate = Convert.ToDateTime(FromDate.Text).ToString("dd-MMM-yyyy");
                tempReportParams.ToDate = Convert.ToDateTime(ToDate.Text).AddMonths(1).AddDays(-1).ToString("dd-MMM-yyyy");
        ...
        private void PageActionHandler()
        {
            try
            {
                if (!usrStockCardFormerFilter_IsPostBack)
                {
                    usrStockCardFormerFilter_IsPostBack = true;
            ...
            try
            {
                switch (controlType)
                {
                    case ControlsEnum.ITEMCATEGORY:
                        dtResult = GenerateReportBL.GetItemCategoryFormer(currentUser.SBUID);
                        break;
                    case ControlsEnum.ITEMS:
                        dtResult = GenerateReportBL.GetItemsByCategoryFormer(Convert.ToInt32(ddlItemCategory.SelectedValue));
                        break;
            ...
            try
            {
                switch (controlType)
                {
                    ...
                    case ControlsEnum.FINYEARDATE:
                        ...
                        {
                            FromDate.Text = Convert.ToDateTime(dtResult.Rows[0]["FORM_DT"]).ToString("MMM-yyyy");
                            ToDate.Text = Convert.ToDateTime(dtResult.Rows[0]["TO_DT"]).ToString("MMM-yyyy");
                            ...
                            hdfToDate.Value = Convert.ToDateTime(dtResult.Rows[0]["TO_DT"]).ToString("MMM-yyyy");
                        }
                        break;
                    case ControlsEnum.CURRENTFINYEAR:
                        if (dtResult != null && dtResult.Rows.Count > 0)
