...
{
    public partial class InProcessReport : MISFilterBase
    {
        #region Variables
        ...
        private void GetFieldValues(ControlsEnum controlType)
        {
            switch (controlType)
            {
                case ControlsEnum.DATECHANGED:
                    dtResult = BusinessLogic.CommonManagement.CommonBL.GetLineDetails(Convert.ToDateTime(PTH_DATE.Text));
                ...
                case ControlsEnum.LINEPKCHANGED:
                    dtResult = BusinessLogic.CommonManagement.CommonBL.GetLineProductDetails(Convert.ToDateTime(PTH_DATE.Text), txtLNE_PK.SelectedValue);
                    txtPTH_PRODUCT.Items.Clear();
