...
var PurchaseOrderConfig = {
    ...
    SavePurchaseOrder: "PurchaseOrderGenerate.do?Action=SavePurchaseOrderTrading", //SPPUR_ORDER_TRD_WKF_SAVE
    ...
    BACKURL: "PurchaseOrderTradingList.aspx",
    InboxURL: "../AccountManagement/WorkflowInbox.aspx",
    PURCHASERQSTENTRYURL: "../PurchaseRequestManagement/PurchaseRequestTrading.aspx",
    UomURL: "MaterialManagement.do?Action=GetItemUOMTrading&MaterialPK=",
    GetConversionFactor: "CommonManagement.do?Action=GetItemUOMConversionFactor",
    // messages
    ...
    HeaderLevelTaxAdded: "Translate(HeaderLevelTaxAdded)",
    HeaderLevelDiscountAdded: "Translate(HeaderLevelDiscountAdded)",
    ItemLevelTaxAdded: "Translate(ItemLevelTaxAdded)",
    ItemLevelDiscountAdded: "Translate(ItemLevelDiscountAdded)",
    AlreadyAddedIn: "Translate(AlreadyAddedIn)",
    SessionExpired: "Translate(Msg_Dept_Session_Expired)",
    NoConversionFactorExistToVendorUOM: "Translate(NoConversionFactorExistToVendorUOM)",
    MultiplePRSameItemDiffUOM: "Translate(MultiplePRSameItemDiffUOM)",
    ...
    // url
    PurchaseOrderListUrl: "PurchaseOrderTradingList.aspx",
...
function PageInit() {
    ...
    ShowHideHeaderTaxDiscount(); // Show/Hide HeaderLevel Tax/Discount Popup Images w. r. to Configuration
    ShowHideLineWiseTaxDiscountTextBoxes(); // Show/Hide HeaderLevel Tax/Discount textboxes  w. r. to Configuration
}
    ...
    $.get(reqString, function (data) {
        ...
        if (data.length > 0) {
            for (var i in data) {
                ...
                var amount = isNaN(hdrAmt) ? 0 : hdrAmt - (isNaN(hdrDisc) ? 0 : hdrDisc);
                //End
...
function BindPendingPRGrid() {
    ...
    var queryString = "&BizUnit=" + PurchaseOrderConfig.BizUnitPk + "&POID=" + $("[id$=PurchaseOrderID]").val() + "&Status=" + $("[id$=SearchType]").val() + "&SearchValue=" + $("[id$=SearchValue]").val() + "&FromDate=" + $("[id$=FromDate]").val() + "&ToDate=" + $("[id$=ToDate]").val() + "&P_DeptPK=" + $("[id$=Store]").val() + "&SortBy=PRD_SORT_DATE" + "&ApplicationPK=" + $("[id$=hdfApplicationPK]").val() + "&ProcessID=" + $("[id$=hdfProcessID]").val();
...
function AddPORequestList() {
    ...
    //------Check if selected PR's have same item with different UOM------------
    if (!IsPRHaveSameItemDifferentUOM()) {
        isValid = false;
        GrandScriptUtils.ShowModal(PurchaseOrderConfig.MultiplePRSameItemDiffUOM.fontcolor("red"), PurchaseOrderConfig.Information);
        return false;
    }
...
function GetVendorRates(vendorID) {
    if (vendorID != "0") {
        ...
        for (var i in summaryPOList) {
            {
                if (summaryPOList[i].IO_NO != null && $.trim(summaryPOList[i].IO_NO) != "") {
                    comments += comments == "" ? ("IO : " + summaryPOList[i].IO_NO) : (", " + summaryPOList[i].IO_NO);
                }
            ...
            VendorRates.ItemDetails.push(new Object(
                {
                    ITM_PK: summaryPOList[i].POR_ITEM,
                    ITM_QTY: summaryPOList[i].POR_QTY_ORDERED  ,
                    ITM_UOM: summaryPOList[i].POR_UOM            
                }));
        }
        ...
        for (var k in summaryPOList) {
            if (summaryPOList[k].PRD_PURPOSE != null && $.trim(summaryPOList[k].PRD_PURPOSE) != "") {
            ...
            if (data != null && data.Rates.length > 0) {
                for (var dataItem in data.Rates) {
                    var purOrderObj = JSLINQ(summaryPOList)
                          ...
                          .FirstOrDefault(null);
                    if (purOrderObj != null) {
                        purOrderObj.ITV_PRICE = parseFloat(data.Rates[dataItem].ITV_PRICE_TRD).toFixed(RateDec);
                        purOrderObj.ITV_PRICE_PREV = parseFloat(data.Rates[dataItem].ITV_PRICE_TRD_PREV).toFixed(RateDec);
                        if (data.Rates[dataItem].ITM_ITV_MOQ_UOM_QTY == null) {
                            purOrderObj.ITM_ITV_MOQ_UOM_QTY = 0;
                            GrandScriptUtils.ShowModal(String.format(PurchaseOrderConfig.NoConversionFactorExistToVendorUOM.fontcolor("red"),purOrderObj.PRD_UOM_TEXT,data.Rates[dataItem].ITV_MOQ_UOM_TEXT), PurchaseOrderConfig.Information);
                            return false;
                        }
                        else {
                            purOrderObj.ITM_ITV_MOQ_UOM_QTY = parseFloat(data.Rates[dataItem].ITM_ITV_MOQ_UOM_QTY).toFixed(QtyDec);
                        }
                        purOrderObj.ITV_MOQ_UOM_TEXT = data.Rates[dataItem].ITV_MOQ_UOM_TEXT;
                        purOrderObj.ITV_MOQ_UOM = data.Rates[dataItem].ITV_MOQ_UOM;
                    }
    ...
    if (vendorID != "0") {
        $.get(PurchaseOrderConfig.GetVendorDetailsByID + vendorID, function (data) {
            if (data.length > 0) {
                ...
                $("[id$=POH_TYPE]").val(data[0].VEN_PO_TYPE);
...
function FillMaterialDetails(materialID) {
    ...
    var DefaultPOItemUOM = $("input[id$=hdfDefaultPOItemUOM]").val();
    $.get(PurchaseOrderConfig.GetVendorMaterialDetails + PurchaseOrderConfig.VendorPK + "&MaterialId=" + materialID, function (data) {
        if (data.length > 0) {
            ...
            //By default the UOM need to be converted into Vendor UOM (Bug ID: 36506 )
            $("[id$=POD_UOM]").val(data[0].ITV_MOQ_UOM);
            FillUOM(materialID, data[0].ITV_MOQ_UOM);      
...
function BindPODetails() {
    ...
    for (var i in summaryPOList) {
        ...
        if (PurOrderObj == null) {
            ...
            PurOrderObj.POD_QTY_REQUESTED = summaryPOList[i].ITM_ITV_MOQ_UOM_QTY; //summaryPOList[i].POR_QTY_ORDERED;
            PurOrderObj.POD_UOM = summaryPOList[i].ITV_MOQ_UOM; //summaryPOList[i].POR_UOM;
            PurOrderObj.UOM_CODE = summaryPOList[i].ITV_MOQ_UOM_TEXT;  //summaryPOList[i].PRD_UOM_TEXT;
            ...
            PurOrderObj.POD_AMOUNT = parseFloat(summaryPOList[i].ITV_PRICE * summaryPOList[i].ITM_ITV_MOQ_UOM_QTY).toFixed(AmtDec);
            PurOrderObj.POD_TAX_PERC = 1;
            PurOrderObj.POD_TAX = 0;
            PurOrderObj.POD_DISC_PERC = 1;
            PurOrderObj.POD_DISC_AMT = 0;
            PurOrderObj.POD_AMT_VALUE = parseFloat(summaryPOList[i].ITV_PRICE * summaryPOList[i].ITM_ITV_MOQ_UOM_QTY).toFixed(AmtDec);
            ...
            PurOrderObj.POD_QTY_REQUESTED = summaryPOList[i].ITM_ITV_MOQ_UOM_QTY;
            PurOrderObj.POD_CONV_FACT = 1;
            PurOrderObj.POD_RATE = parseFloat(summaryPOList[i].ITV_PRICE).toFixed(RateDec);
            if (!isNaN(summaryPOList[i].ITV_PRICE_PREV))
                PurOrderObj.POD_RATE_PREV = parseFloat(summaryPOList[i].ITV_PRICE_PREV).toFixed(RateDec);
            PurOrderObj.POD_AMOUNT = summaryPOList[i].ITV_PRICE * summaryPOList[i].ITM_ITV_MOQ_UOM_QTY;
            PurOrderObj.POD_AMT_VALUE = summaryPOList[i].ITV_PRICE * summaryPOList[i].ITM_ITV_MOQ_UOM_QTY;
        }
    }
...
function AddPODetails() {
    ...
    if ($(document.forms[0]).valid()) {
        ...
        if (PurOrderObj == null) {
            ...
            PurOrderObj.POD_UOM = $("[id$=POD_UOM]").val();
            PurOrderObj.UOM_CODE = $("[id$=POD_UOM] :selected").text(); //$("[id$=POD_UOM]").val();
            ...
            if ($("[id$=POH_IS_AMEND]").val() == "1") {
                ...
                PurOrderObj.POD_REQD_DATE = $("[id$=POD_REQD_DATE]").val();
                PurOrderObj.POD_UOM = $("[id$=POD_UOM]").val();
                PurOrderObj.UOM_CODE = $("[id$=POD_UOM] :selected").text();
                $("#divData").data("ReqPODetails", PurOrderDetails);
                UpdateLineItemTax(currSlNo, "", PurOrderObj.POD_RATE);
                GrandGrid.MakeGrid($("#grdPODetails"), 0, PurOrderDetails);
                ClearPODetails();
...
function BindPRGridPopupAmend(POD_PK) {
    ...
    if (PurOrderListArray.length > 0) {
        ...
        $("#divAmendPRDetails").dialog(
                {
                    width: 800,
                    title: "Translate(AmendPRDetails)"
                });
    }
    else {
...
function AddLineItemTax(slNo, amount) {
    ...
    //Check tax is  added in header level
    var TaxDet = $("#divData").data("TaxDetails");
    if (TaxDet.length > 0) {
        if (IsTaxDiscAddedInHeaderOrItem(TaxDet, 1,0)) {
            return false;
        }
    }
...
function AddLineItemRate(slNo, qty, itempk, isRateUpdate,rateChangeReason,itemUOMPK) {
    ...
    $("select[id$=ddlItemUOMPopup]").rules("add", {
        selectNone: true,
        messages: { selectNone: "Translate(PleaseSelectUnit)" }
    });
    $("[id$=hdfCurrItemUomPKPopup]").val(itemUOMPK);
    $("[id$=hdfCurrItemPKPopup]").val(itempk);
    var prQty = parseFloat($("#POD_H_QTY_REQUESTED_" + slNo).val());
    $("[id$=hdfCurrentItemPRqty]").val(prQty);
    FillItemUOMPopup(itempk, itemUOMPK);
}
...
function AddItemHeaderDiscount() {
    ...
    //Check Discount is  added in Item level
    var TaxDet = $("#divData").data("TaxDetails");
    if (TaxDet.length > 0) {
        if (IsTaxDiscAddedInHeaderOrItem(TaxDet, 3,1)) {
            return false;
        }
    }
...
function AddItemHeaderTax() {
    ...
    //Check Tax is added in Item level
    var TaxDet = $("#divData").data("TaxDetails");
    if (TaxDet.length > 0) {
        if (IsTaxDiscAddedInHeaderOrItem(TaxDet, 1,1)) {
            return false;
        }
    }
        ...
        $("#grdPODetails tr:has(td)").each(function (index) {
            ...
            if (slNo != null && parseInt(slNo) == parseInt(PurchaseOrderConfig.ItemTaxPK)) {
                ...
                if (parseInt($("[id$=IsLineDiscount]").val()) == 1) {
                    ...
                    if ($("[id$='hdfIsItemTaxForPurchase']").val() == "1") {
        ...
        if (isNaN(parseFloat($("[id$=TaxRate]").val())) || parseInt(parseFloat($("[id$=TaxRate]").val())) < 0) {
            ...
            for (var i in PurOrderDetails) {
                if (PurOrderDetails[i].POD_SL_NO == slNo) {
                    ...
                    PurOrderDetails[i].POD_UOM = $("[id$=ddlItemUOMPopup]").val();
                    PurOrderDetails[i].UOM_CODE = $("[id$=ddlItemUOMPopup] :selected").text();
                    PurOrderDetails[i].POD_QTY_REQUESTED = parseFloat($("[id$=ItemQty]").val());
                    selectedItemId = PurOrderDetails[i].POD_ITEM;     
                }
            }

            //Update Request List also
            var fromUOMPK = $("[id$=hdfCurrItemUomPKPopup]").val();
            var toUOMPK = $("[id$=ddlItemUOMPopup]").val();
            var itemPk = selectedItemId;
            var qty = 0;
            var convFactor = 0;
            var convertedQty = 0;
            $.get(PurchaseOrderConfig.GetConversionFactor + "&ItemID=" + itemPk + "&FromUOM=" + fromUOMPK + "&ToUOM=" + toUOMPK, function (data) {
                if (data) {
                    convFactor = isNaN(data) ? 0 : data;
                    var PurOrderList = $("#divData").data("ReqPOList");
                    for (var j in PurOrderList) {
                        if (PurOrderList[j].POR_ITEM == selectedItemId) {
                            convertedQty = PurOrderList[j].PRD_QTY_APPROVED * convFactor;
                            PurOrderList[j].PRD_QTY_APPROVED = parseFloat(convertedQty).toFixed(QtyDec);
                            PurOrderList[j].PRD_QTY_ORDERED = parseFloat(PurOrderList[j].PRD_QTY_ORDERED * convFactor).toFixed(QtyDec);
                            PurOrderList[j].PRD_QTY_BALANCE = parseFloat(PurOrderList[j].PRD_QTY_BALANCE * convFactor).toFixed(QtyDec);
                            PurOrderList[j].PRD_UOM_TEXT = $("[id$=ddlItemUOMPopup] :selected").text();
                            PurOrderList[j].POR_QTY_ORDERED = parseFloat(PurOrderList[j].POR_QTY_ORDERED * convFactor).toFixed(QtyDec);
                            PurOrderList[j].POR_QTY_ADDITIONAL = parseFloat(PurOrderList[j].POR_QTY_ADDITIONAL * convFactor).toFixed(QtyDec);
                            PurOrderList[j].POR_UOM = $("[id$=ddlItemUOMPopup]").val()
                        }
                    }                   
                }
            });            
            //End Update Request List also
...
function ResetPage() {
    ...
    window.location = $("[id$=hdfBackUrl]").val();// PurchaseOrderConfig.PurchaseOrderListUrl; 
    return false;
}
...
function FillPurchaseOrder(tr) {
    ...
    var itemPK = 0,UOMPK=0;
    itempk = GrandGrid.Utilities.GetColumnValue(tr, PurchaseOrderConfig.POD_ITEM, $(tr).parents("table:first").attr("id"));
    UOMPK = GrandGrid.Utilities.GetColumnValue(tr, PurchaseOrderConfig.POD_UOM, $(tr).parents("table:first").attr("id"));
    FillUOM(itempk, UOMPK);
...
function ViewSelectedPR(itemPK) {
    ...
    $("#divPurchaseRequest").dialog({ "width": 500 });
}
...
function AfterGridBind(grdID) {
    ...
    if (grdID == "grdPendingPRList") {
        ...
        $("#grdPendingPRList tr:has(td)").each(function (index) {
            ...
            //Providing hyperlink to open PR in New Window
    ...
    if (grdID == "grdPODetails") {
        ...
        $("#grdPODetails tr:has(td)").each(function (index) {
            ...
            if (colIndex != null && $("[id$='hdfIsItemTaxForPurchase']").val() == "1") {
            ...
            if (colIndex != null && $("[id$='hdfIsItemDiscountPurchase']").val() == "1") {
            ...
            if (colIndex != null) {
                ...
                itemUOMPK = GrandGrid.Utilities.GetColumnValue($(this), PurchaseOrderConfig.POD_UOM, $(this).parents("table:first").attr("id"));
        ...
        $("#grdPODetails tr:has(th)").each(function (index) {
            ...
            if (ColHeadTaxIndex != null && $("[id$='hdfIsItemTaxForPurchase']").val() == "1") {
            ...
            if (ColHeadDiscIndex != null && $("[id$='hdfIsItemDiscountPurchase']").val() == "1") {
...
function GridMaterialAction(tr, command) {
    ...
    switch (command.toString().toUpperCase()) {
        case PurchaseOrderConfig.HistoryCommand:
            ...
            var toUOMPK = GrandGrid.Utilities.GetColumnValue(tr, PurchaseOrderConfig.POD_UOM, $(tr).parents("table:first").attr("id"));
            ShowRateHistory(materialID, toUOMPK);
            break;
...
function ShowRateHistory(itemPK, toUOMPK) {
    //<summary>function used to show rate history</summary>
    ///////////////
    ajaxurl = PurchaseOrderConfig.GetItemRates + "&ItemPK=" + itemPK + "&ToUOM=" + toUOMPK;
...
function AddLineItemDiscountDtl(slNo, amount) {
    ...
    var TaxDet = $("#divData").data("TaxDetails");
    if (TaxDet.length > 0) {
        if (IsTaxDiscAddedInHeaderOrItem(TaxDet, 3,0)) {
            return false;
        }
    }
...
function Print_PR(prID) {
    var url = PurchaseOrderConfig.PURCHASEREQUESTREPORTURL + "?ID=" + prID + "&APPTYPE=PRT";
    OpenPDF(url);
}
...
function IsSameRequestedDept() {
    ...
    $("[id$=POH_ISSUE_DEPT]").val(issueDeptAdded);  //Setting Issue dep for furthure use;
    $("[id$=hdfIssueDeptText]").val(issueDeptAddedText);
    return flag;
}
// #region Direct PO
...
function FillUOM(matID, selectVal) {
    ///<summary>function To Fill UOM Details </summary>   
    var drpID = $("select[id$=POD_UOM]").attr("id");
    $.get(PurchaseOrderConfig.UomURL + matID, function (data) {
        GrandScriptUtils.FillDropDown(drpID, data, true, true, selectVal);

    });
}
function FillItemUOMPopup(matID, selectVal) {
    ///<summary>function To Fill UOM Details </summary>  
    var drpID = $("select[id$=ddlItemUOMPopup]").attr("id");
    $.get(PurchaseOrderConfig.UomURL + matID, function (data) {
        GrandScriptUtils.FillDropDown(drpID, data, true, true, selectVal);

    });
}
function ConvertQtyBasedOnUOM() {
    var fromUOMPK = $("[id$=hdfCurrItemUomPKPopup]").val();
    var toUOMPK = $("[id$=ddlItemUOMPopup]").val();
    var itemPk = $("[id$=hdfCurrItemPKPopup]").val();
    var qty = 0;
    var convFactor = 0;
    var convertedQty = 0;
    qty = parseFloat($("[id$=hdfCurrentItemPRqty]").val());
    $.get(PurchaseOrderConfig.GetConversionFactor + "&ItemID=" + itemPk + "&FromUOM=" + fromUOMPK + "&ToUOM=" + toUOMPK, function (data) {
        if (data) {          
            convFactor = isNaN(data) ? 0 : data;
            convertedQty = qty * convFactor;
            $("[id$=ItemQty]").val(parseFloat(convertedQty).toFixed(QtyDec));           
        }
    });
}

function ShowHideHeaderTaxDiscount() {
    //<summary>Function Used to Show/Hide HeaderLevel Tax/Discount Popup Images w. r. to Configuration </summary>
    if ($("[id$=hdfIsHeaderTaxForPurchase]").val() == "0") {
        $("#imgHeaderTax").hide();
    }
    if ($("[id$=hdfIsHeaderDiscForPurchase]").val() == "0") {
        $("#imgHeaderDiscount").hide();
    }
}
function ShowHideLineWiseTaxDiscountTextBoxes() {
    //<summary>Function Used to Show/Hide line wise tax and discount textboxes w. r. to Configuration </summary>
    if ($("[id$='hdfIsItemDiscountPurchase']").val() == "0") {
        $("#tblPODetails tr").each(function (index) {
            $(this).find("th:eq(" + 5 + ")").hide();
            $(this).find("td:eq(" + 5 + ")").hide();            
        });
    }
    if ($("[id$='hdfIsItemTaxForPurchase']").val() == "0") {
        $("#tblPODetails tr").each(function (index) {
            $(this).find("th:eq(" + 6 + ")").hide();
            $(this).find("td:eq(" + 6 + ")").hide();

            $(this).find("th:eq(" + 0 + ")").width("55%"); //For design adjustment.(due to the hiding of tax & discount)
            $("[id$=txtItemCode]").addClass("input-w87-8per");
        });
    }
}

function IsTaxDiscAddedInHeaderOrItem(TaxDetailsObj,category,Isheader) {
    //<summary>Function used to check  tax/disc  is already added in header level/Item level</summary>
    var IsAlreadyHave = new Object();
    var msg1 = "", msg2 = "";   
    if (Isheader == 0) {//From Line Item        
        IsAlreadyHave = JSLINQ(TaxDetailsObj)
                          .Where(function (tax) { return tax.IsHeader == true && tax.POT_TAX_CATEGORY==category; })
                          .FirstOrDefault(null);
        msg2 = " header level";
    }
    else {
        IsAlreadyHave = JSLINQ(TaxDetailsObj)
                          .Where(function (tax) { return tax.IsHeader == false && tax.POT_TAX_CATEGORY == category; })
                          .FirstOrDefault(null);
        msg2 = " item level";
    }
    if (IsAlreadyHave == null) {
        ...
        if (category == 1) { msg1 = "Tax"; }
        else { msg1 = "Discount"; }
        GrandScriptUtils.ShowModal(String.format(PurchaseOrderConfig.AlreadyAddedIn.fontcolor("red"), msg1, msg2), PurchaseOrderConfig.Information);
        return true;
    }
}

//<summary>function to check if selected PR's have same item with different UOM </summary>
function IsPRHaveSameItemDifferentUOM() {
    var flag = true;
    var selectedPRUOM = 0;
    var selectedPRItem = 0;

    $("#grdPendingPRList tr:has(td)").each(function () {
        grdID = $(this).parents("table:first").attr("id");
        if ($(this).find("td:first").find("input[type=checkbox]").attr("checked")) {
            if (selectedPRUOM == 0) {
                selectedPRUOM = GrandGrid.Utilities.GetColumnValue($(this), PurchaseOrderConfig.PRD_UOM, grdID);
                selectedPRItem = GrandGrid.Utilities.GetColumnValue($(this), PurchaseOrderConfig.PRD_ITEM, grdID);
            }
            else {
                if (selectedPRItem == GrandGrid.Utilities.GetColumnValue($(this), PurchaseOrderConfig.PRD_ITEM, grdID) && selectedPRUOM != GrandGrid.Utilities.GetColumnValue($(this), PurchaseOrderConfig.PRD_UOM, grdID)) {
                    flag = false;
                    return flag;
                }
            }
            //Check already added list also
            var PurOrderList = $("#divData").data("ReqPOList");
            if (PurOrderList != null && PurOrderList.length > 0) {
                var PurOrderObj;
                PurOrderObj = JSLINQ(PurOrderList)
                                   .Where(function (item) { return item.POR_ITEM == selectedPRItem && item.POR_UOM != selectedPRUOM; })
                                   .FirstOrDefault(null);
                if (PurOrderObj == null) {
                }
                else {
