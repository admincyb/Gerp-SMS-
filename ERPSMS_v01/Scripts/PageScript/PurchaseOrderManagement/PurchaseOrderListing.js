...
var POListing = {
    ...
    CreatePO: "PurchaseOrderGenerate.aspx",
    ...
    PAGEURL: "/PurchaseOrderManagement/PurchaseOrderGenerate.aspx?TYPE=1",
    ...
    POHNO: "POH_NO",
    IsCommentExist: "POH_IS_CMNT_EXISTS",
...
//    $("[id$=SearchValue]").val("");
//    GrandScriptUtils.AddDateRange("FromDate", "hdfFrmDate", "ToDate", "hdfToDate", false, false, false, false);
//   
//    if (strname == "0") {
//        $("#divSearchDtls").hide();
//        $("#divDate").hide();
...
function AfterGridBind() {
    ...
    //<summary>Function Used Hide/Show imbShortClose Button</summary>
    var isClosePO = $("[id$=hdnClosePO]").val();
    ...
    $("#grdPODetails").find("tr:has(td)").each(function () {
        ...
        //            
        //        }
        var ColIndex = GrandGrid.Utilities.GetColumnIndex($(this), "SOH_NO", tableID);
        ...
        var isCommented = GrandGrid.Utilities.GetColumnValue(this, POListing.IsCommentExist, tableID);
        if (UserStatus == 1) {
        ...
        if (isCommented == 1) {
            $(this).find("td:last input[id$=imbNoComments]").hide();
            $(this).find("td:last input[id$=imbComnts]").show();
        }
        else {
            $(this).find("td:last input[id$=imbNoComments]").show();
            $(this).find("td:last input[id$=imbComnts]").hide();
        }
        ...
        //Show tooltip against  item details in listing.
        var ItemIndex = 0;
        ItemIndex = GrandGrid.Utilities.GetColumnIndex($(this), "POH_ITEM_TEXT", tableID);
        ItemDetails = GrandGrid.Utilities.GetColumnValue($(this), "POH_ITEM_FULLTEXT", tableID);
        if (ItemIndex != null) {
...
function GridHandler(tr, command) {
    ...
    switch (command.toString()) {
        ...
        case POListing.SHORTCLOSE:
            ...
            ResetShortClose();
            $("[id$=lblPOH_NO]").html(GrandGrid.Utilities.GetColumnValue(tr, POListing.POHNO, $(tr).parent().attr("id")));
            $("[id$=POID]").val(POID);
            $("#divShortClose").dialog("open");
            $("#divShortClose").dialog({ width: 500, height: 200, resizable: false });
            break;
        case POListing.Comments:
            $("[id$=hdfCmntTrxNo]").val(GrandGrid.Utilities.GetColumnValue(tr, POListing.POHNO, $(tr).parent().attr("id")));
            $("[id$=hdfCmntTrxPk]").val(GrandGrid.Utilities.GetColumnValue(tr, POListing.POID, $(tr).parent().attr("id")));
            $("[id$=btnComments]").click();
            break;
    }
    return false;
}
...
function FillCompanyDisplayNames() {
    ...
    $.get(POListing.PurchaseSearchAutoCompleteURL + "&SearchType=CMP_DISPLAY_CODE" + "&PageURL=" + POListing.PAGEURL, function (data) {
