...
var CompoundMaster = {
    ...
    GetConversionUOMListURL: "CompoundMaster.do?Action=GetUOMListForItem&MatPK=",
...
function AddMaterialDetails() {
    ...
    if ($(document.forms[0]).valid()) {
            ...
            if (flag) {
                ...
                obj.CPD_DRY_QTY = $("[id$=DRYWEIGHT]").html();
...
function FillMaterialConversionUOM(selValue) {
    ...
    var drpID = $("select[id$=CPD_QTY_UOM]").attr("id");
    $.get(CompoundMaster.GetConversionUOMListURL + $("select[id$=CPD_ITEM]").val() + "&Catg=" + $("select[id$=CPD_ITEM_CATEGORY]").val(), function (data) {
    ...
    if ($(document.forms[0]).valid()) {
        if (CompoundJson.CompoundMaterialsList.length > 0) {
            ...
            if (CheckQuantityMatch()) {
                ...
                GrandScriptUtils.ShowModal("Unit Quantity not matching with Material Compound", CompoundMaster.InformationTitle);
            }
        }
        else {
...
function CalculateDryWeight() {
    if ($('select[id$=CPD_WET_QTY]').val() != "0") {
        ...
        if (isNaN(dryQty) || Infinity == dryQty || dryQty == 0) {
            ...
            dryQty = Round(dryQty, 3);
            $("[id$=DRYWEIGHT]").html(dryQty);
