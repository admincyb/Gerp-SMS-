...
var RequisitionList = {
    ...
    REDIRECTURLEDIT: "../StoreManagement/ExternalMaterialReceive.aspx?IssueID=",
    ADDNEWURL: "../StoreManagement/ExternalMaterialReceive.aspx",
    PRINTURL: "../StoreManagement/ExternalMaterialReceiveReport.aspx",
    REPORTURL: "../Reports/GenerateReport.aspx",
    PAGEURL: "../StoreManagement/ExternalMaterialReceive.aspx",
...
function BindGrid() {
    ...
    var ajaxUrl = RequisitionList.BindGridURL + $("[id$=ddlTrnStatus]").val() + "&IssueNo=" + $("[id$=SearchValue]").val() + "&ISS_TYPE=" + $("select[id$=ddlType]").val() + "&ISS_TO=" + $("[id$=ICH_ISS_RCV_PK]").val() + "&ISS_STORE=" + $("[id$=ddlIssuingStore]").val() + "&LotNo=" + $("[id$=txtLotno]").val() + "&RefNo=" + $("[id$=txtRefNo]").val() + "&BizUnit=" + $("[id$=BizUnitPk]").val() + "&ProcID=" + $("[id$=hdfProcId]").val() + "&TYPE=" + TRX_TYPE + "&PageUrl=" + RequisitionList.PAGEURL + "&FromDate=" + $("[id$=SearchFromDate]").val() + "&ToDate=" + $("[id$=SearchToDate]").val() + "&SortColumn=" + $("[id$=hdfSortColumn]").val() + "&SortOrder=" + $("[id$=hdfSortOrder]").val() + "&SortColumn1=" + $("[id$=hdfSortColumn1]").val() + "&SortOrder1=" + $("[id$=hdfSortOrder1]").val() + "&CatPk=" + $("[id$=hdfItemCatPK]").val() + "&ItmPk=" + $("[id$=hdfItemPk]").val() + "&CMP_PK=" + $("[id$=ddlPlantCode]").val();
...
function AfterGridBind() {
    ...
    $("#grdRequsitionList").find("tr:has(td)").each(function () {
        ...
        if ($("[id$=hdfCancelFlag]").val() == 1 && UserStatus != 0 && UserStatus != 4) {
            $(this).find("td:last input[id$=imbCancel]").show();
        }
        else {
            $(this).find("td:last input[id$=imbCancel]").hide();
        }
        if ($("[id$=hdnModifyFlag]").val() == 1 && UserStatus == 1 && UserStatus != 4) {   //UserStatus=> 4 Canceled
        ...
        if ($("[id$=hdfDeptID]").val() != data) {
            ...
            switch (command.toString()) {
                ...
                case RequisitionList.PerformAction:
                    if (TRX_TYPE == 4) {
                        window.location = RequisitionList.ADDNEWURL + "?IssueID=" + issuingHeaderId + "&Status=0" + "&TYPE=4";
                    }
                    else {
                ...
                case RequisitionList.MODIFY:
                    if (TRX_TYPE == 2) { //EMR         
                        window.location = RequisitionList.ADDNEWURL + "?IssueID=" + issuingHeaderId + "&Status=1&IsModify=1&TYPE=2";
                    }
                    break;
