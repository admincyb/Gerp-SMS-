...
var RequisitionSlip = {
    ...
    REDIRECTURLAFTERSAVE: "../StoreManagement/ExternalMaterialIssueListDiscrete.aspx",
...
function FillBatchNoAutoComplete() {
    ...
    GrandScriptUtils.MakeAutoComplete("BatchNo", RequisitionSlip.FillBatchNoDropDownURL + $("[id$=BizUnitPk]").val() + "&MaterialID=" + $("[id$=MaterialPK]").val() + "&DepartmentID=" + $("select[id$=ICH_DEPT]").val(), "BatchNoPK", true, false, "BizUnitPk", true);
}
...
function FillCategoryMaterials(categoryID) {
    ...
    GrandScriptUtils.MakeAutoCompleteLimitLen("Material", RequisitionSlip.MaterialURL, "MaterialPK", true, false, "MaterialCategoryPK", true, "Store", "", "", $("[id$=AutoStartValue]").val());
}
///#endregion
...
function FillIssuingToList(issueTypeID, issueTypeText) {
    ...
    // GrandScriptUtils.MakeAutoCompleteLimitLen("txtIssueTo", RequisitionSlip.FillIssuingToList + $("[id$=BizUnitPk]").val() + "&issuingType=" + issuingType + "&Despatch=1", "ICH_ISS_RCV_PK", true, "Select", "", "", $("[id$=AutoStartValue]").val());
    GrandScriptUtils.MakeAutoCompleteLimitLen("txtIssueTo", RequisitionSlip.FillIssuingToList + $("[id$=BizUnitPk]").val() + "&issuingType=" + issuingType + "&Despatch=1", "ICH_ISS_RCV_PK", true, false, "txtIssueTo", true, "Select", "", "", $("[id$=AutoStartValue]").val(), afterAutoComplete);
    ...
    if (grdID == "grdRequisitionSlip") {
        ...
        $("#grdRequisitionSlip").find("tr:has(td)").each(function (index) {//loop through each td and find the remarks is null if null it will be cleared
            ...
            if (colIndex != null) {
                ...
                $(this).find("td:eq(" + colIndex + ")").html(parseFloat(colVal).toFixed(QtyDec));
            }
...
function AddRequisitionDetails() {
    ...
    if ($(document.forms[0]).valid()) {
        ...
        if (parseInt(editRequisition) == 0) {
            ...
            for (var i in ObjRequisition.ConsumptionDtl) {
                if (ObjRequisition.ConsumptionDtl[i].ICD_ITEM == $("[id$=MaterialPK]").val() && parseInt(editRequisition) != ObjRequisition.ConsumptionDtl[i].ICD_ITEM)
                    flag = false;
                break;
            }
...
function FillDetails(tr) {
    ...
    $("[id$=CurrentStock]").html(parseFloat(GrandGrid.Utilities.GetColumnValue(tr, RequisitionSlip.CurrentStock, $(tr).parent().attr("id"))).toFixed(QtyDec));
    $("input[id$=ICD_QTY_CONSUMED]").val(parseFloat(GrandGrid.Utilities.GetColumnValue(tr, RequisitionSlip.MaterialQtyRequest, $(tr).parent().attr("id"))).toFixed(QtyDec));
