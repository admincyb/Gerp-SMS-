...
var RequisitionSlip = {
    ...
    REDIRECTURLAFTERSAVE: "../StoreManagement/ExternalMaterialIssueList.aspx",
    ...
    if (targetControlID == "BatchNo") {
        ...
        $.getJSON(RequisitionSlip.FillBatchDetailGetURL + $("[id$=BizUnitPk]").val() + "&BatchID=" + batchID, function (data) {
            if (data != null) {
                ...
                if (batchID != 0) {
                    //alert(QtyDec);
                    $("[id$=CurrentStock]").html(GrandScriptUtils.FormatDecimalWithoutRounding(parseFloat(data[0].SBD_QTY_IN_STOCK),QtyDec));
    ...
    if (grdID == "grdRequisitionSlip") {
        ...
        $("#grdRequisitionSlip").find("tr:has(td)").each(function (index) {//loop through each td and find the remarks is null if null it will be cleared
            ...
            if (colIndex != null) {
                ...
                $(this).find("td:eq(" + colIndex + ")").html(GrandScriptUtils.FormatDecimalWithoutRounding(parseFloat(colVal), QtyDec));
            }
