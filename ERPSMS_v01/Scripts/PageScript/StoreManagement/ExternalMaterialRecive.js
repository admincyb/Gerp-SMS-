...
var RequisitionSlip = {
    ...
    FillMaterialCategoryExceptFGAutoList: "MaterialCategory.do?Action=GetMaterialCategoryListExceptFG&AUTOSEARCH=1&SBUPk=",
    ...
    REDIRECTURLAFTERSAVE: "../StoreManagement/ExternalMaterialReceiveList.aspx",
    ...
    MaterialURL: "MaterialManagement.do?Action=GetMaterialByCategoryAndStore&SBUPk=",
    ...
    EMRAlreadyUsed: "Translate(EMRAlreadyUsed)",
...
function FillMaterialCategoryAutoComplete() {
    ...
    GrandScriptUtils.MakeAutoComplete("MaterialCategory", RequisitionSlip.FillMaterialCategoryExceptFGAutoList + $("[id$=BizUnitPk]").val() + "&ItemType=" + $("select[id$=ICH_ITEM_TYPE]").val() + "&Store=" + $("select[id$=ICH_DEPT]").val(), "MaterialCategoryPK", true, false, "BizUnitPk", true);
}
...
function AfterAutoCompleteSelect(targetControlID) {
    ...
    else if (targetControlID == "Material") {
        RemoveValidations();
        AddValidations(1);     
        GetCurrentStock($("[id$=MaterialPK]").val()); //No need to fill the current stock here. It should based on the batch no.
    }
}
...
function ResetGridControlDetails() {
    ...
    $("input[id$=MaterialCategory]").val("Select/Type");
    $("input[id$=MaterialCategoryPK]").val("0");
    $("input[id$=Material]").val(typeText);
    $("input[id$=MaterialPK]").val("0");
...
function FillCategoryMaterials(categoryID) {
    ...
    GrandScriptUtils.MakeAutoCompleteLimitLen("Material", RequisitionSlip.MaterialURL + $("[id$=BizUnitPk]").val() + "&CategoryID=" + categoryID + "&Store=" + $("select[id$=ICH_DEPT]").val(), "MaterialPK", true, false, "MaterialCategoryPK", true, "ICH_DEPT", "", "", $("[id$=AutoStartValue]").val(), "", typeText, false);
}
...
function AfterGridBind(grdID) {
    ...
    if (grdID == "grdRequisitionSlip") {
        ...
        if (RequisitionSlip.IsModifyEMR == true) { $("[id$=btnSave]").hide(); }
    ...
    var category = $("[id$=MaterialCategory]").val();
    FillMaterialCategoryAutoComplete();
    $("[id$=MaterialCategoryPK]").val(categoryid);
    $("[id$=MaterialCategory]").val(category);
    FillCategoryMaterials($("[id$=MaterialCategoryPK]").val());
}
    ...
    if ($(document.forms[0]).valid()) {
        if (ObjRequisition.ConsumptionDtl.length > 0) {
            ...
            $.post(RequisitionSlip.RequisitionSaveURL, jSonString, function (data) {///if data=0 already exist if data==1 saved successfully
                ...
                else if (parseInt(data[0]) == -42) {//already used
                    GrandScriptUtils.ShowModal(RequisitionSlip.EMRAlreadyUsed, RequisitionSlip.MessageBoxTitle);
                    $("[id$=SubmitFlag]").val('0')
                }
                else {
...
function AddRequisitionDetails() {
    ...
    if ($(document.forms[0]).valid()) {
        ...
        if (flag) {
            ...
            //obj.ICD_EXPIRY_DATE = $("input[id$=ICD_EXPIRY_DATE]").val() == "" ? "" : $("input[id$=ICD_EXPIRY_DATE]").val();
            ...
            ClearProductDetails();
            //ClearGridControlDetails();
            //FillMaterialCategory(obj.ICD_ITEM_CATEGORY);
            FillMaterialCategoryAutoComplete();
            $("[id$=MaterialCategoryPK]").val(obj.ICD_ITEM_CATEGORY);
            $("[id$=MaterialCategory]").val(obj.ICD_ITEM_CATEGORY_TEXT);
