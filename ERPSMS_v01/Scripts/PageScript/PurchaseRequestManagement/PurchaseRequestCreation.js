...
var PurchaseRequestConfig = {
    ...
    CustomerItemRequestGetGridURL: "PurchaseRequest.do?Action=CustomerItemRequestGet",
    ...
    SavePurchaseRequest: "PurchaseRequest.do?Action=SavePurchaseRequestList",
    ...
    MaterialURL: "MaterialManagement.do?Action=GetMaterialSearchValueByCategoryAndStore&AUTOSEARCH=1&Active=1",
    ...
    FillIODropdownURLAllSBU: "PurchaseRequest.do?Action=GetIONumberAuto",
    ...
    PURCHASEREQUESTURL: "/PurchaseRequestManagement/PurchaseRequestCreation.aspx",
    ...
    PRReqQtySCExceed: "Translate(PRReqQtySCExceed)",
    POExist: "Translate(POExist)",
    ...
    ThisSCWillNotShow: "Translate(ThisSCWillNotShow)",
    ...
    Err_MaxOrderQty: "Translate(Err_MaxOrderQty)",
    BudgetValidationMsg: "Translate(BudgetValidationMsg)",
...
var SelPurpose;
var SCQtyTotal = 0.0;
var SCPrevReqQtyTotal = 0.0;
var SCReqQtyTotal = 0.0;
...
function PageInit() {
    ...
    $("[id$=chkPRCompleted]").attr('checked', false);
    $("[id$=chkShowAllSC]").attr('checked', false);
    ...
    if ($("[id$=hdfEnableGlovePR]").val() == "1") {
        $("[id$=divGloveRequest]").show();
        $("[id$=divPRCompleted]").show();

    }
    else {
        $("[id$=divGloveRequest]").hide();
        $("[id$=divPRCompleted]").hide();
    }
    ...
    if ($("[id$=PurchaseRequestList]").val() == "null") {
        ...
        if (PurchaseRequestConfig.BizUnitPk == $("[id$=hdfPRGloveReqInSBU]").val() && $("[id$=hdfEnableGlovePR]").val() == "1") {
            $("[id$=chkIsGloveRequest]").attr('checked', true);
        }
        else {
            $("[id$=chkIsGloveRequest]").attr('checked', false);
        }
        ...
        if ((purchaseReqObj.PRH_PK != undefined && purchaseReqObj.PRH_PK > 0) || ((purchaseReqObj.PRH_PK != undefined && purchaseReqObj.PRH_PK > 0) || (purchaseReqObj.PRH_PMH_PK != undefined && purchaseReqObj.PRH_PMH_PK > 0) || (purchaseReqObj.MPRH_PMH_PK != undefined && purchaseReqObj.MPRH_PMH_PK > 0) || $("[id$=hdfIsReqDeptPostback]").val() == "1")) {
    ...
    if ($("[id$=hdfEnableGlovePR]").val() == "1") {
        if ($("[id$=chkIsGloveRequest]").is(":checked") == true) {

            DisableAuto($("[id$=MaterialCategory]"), $("[id$=MaterialCategoryPK]"));
            DisableAuto($("[id$=ItemCodeMaterial]"), $("[id$=MaterialPK]"));
            $("[id$=imbMaterialShow]").hide();
            $("[id$=imbAddNewMaterials]").hide();
        }
        else {
            EnableAuto($("[id$=MaterialCategory]"), $("[id$=MaterialCategoryPK]"));
            EnableAuto($("[id$=ItemCodeMaterial]"), $("[id$=MaterialPK]"));
    ...
    var type = $("[id$=PRH_TYPE]").val();
    if (type == "5") {// Glove request
        $("[id$=chkIsGloveRequest]").attr('checked', true);
        $("[id$=chkIsGloveRequest]").attr("disabled", "disabled");
        CheckChangeGloveRequest();
    } else if (type == "6") {
        DisableAuto($("[id$=txtSO]"), $("[id$=PRH_SO_HDR]"));
        $("[id$=chkShowAllSC]").attr("disabled", "disabled");
        $("[id$=divGloveRequest]").hide();
        $("[id$=divPRCompleted]").hide();
    }

    var modifyVal = getQueryStringValue("Modify").toLowerCase();
    if (modifyVal == "1") {
        DisableAuto($("[id$=MaterialCategory]"), $("[id$=MaterialCategoryPK]"));
        $("[id$=hdfIsPRModify]").val("1");
        $("[id$=btnSave").show();
    }
}
...
function CheckChangeGloveRequest() {

    if ($("[id$=hdfProductAddItemDisable]").val() == "1") {
        DisableAuto($("[id$=MaterialCategory]"), $("[id$=MaterialCategoryPK]"));
        DisableAuto($("[id$=ItemCodeMaterial]"), $("[id$=MaterialPK]"));
        $("[id$=imbMaterialShow]").hide();
        $("[id$=imbAddNewMaterials]").hide();
    ...
    if ($("[id$=hdfIsShowDispatchedSCInPR]").val() == "1") {
        IsDispatched = "0"; //Showing
    }
    ...
    if ($("[id$=hdfIsPR_SCinBothSBU]").val() == "0") {
        var sbuID = $("[id$=PRH_SO_BIZUNIT]").val();
        if (sbuID != "0") {
            GrandScriptUtils.MakeAutoComplete("txtSO", PurchaseRequestConfig.FillIODropdownURL + sbuID + "&PrhPK=" + $("[id$=PRH_PK]").val() + "&SohPK=0" + "&isGlove=" + isGlove + "&IsDispatched=" + IsDispatched, "PRH_SO_HDR", true, false, "BizUnitPk", true);
        }
        else {
            GrandScriptUtils.MakeAutoComplete("txtSO", PurchaseRequestConfig.FillIODropdownURL + $("[id$=BizUnitPk]").val() + "&PrhPK=" + $("[id$=PRH_PK]").val() + "&SohPK=0" + "&DeptPk=" + $("[id$=hdfDeptID]").val() + "&isGlove=" + isGlove + "&IsDispatched=" + IsDispatched, "PRH_SO_HDR", true, false, "BizUnitPk", true);
        }
    }
    else {

        GrandScriptUtils.MakeAutoComplete("txtSO", PurchaseRequestConfig.FillIODropdownURLAllSBU + "&PrhPK=" + $("[id$=PRH_PK]").val() + "&SohPK=0" + "&isGlove=" + isGlove + "&IsDispatched=" + IsDispatched, "PRH_SO_HDR", true, false, "BizUnitPk", true);
    }
    if ($("[id$=PRH_PK]").val() > 0) {
        $("[id$=txtSO]").val($.parseJSON($("[id$=PurchaseRequestList]").val()).PRH_SO_NO);
    }
    return false;
}
function ShowAllSC() {
    ...
    if ($("[id$=chkShowAllSC]").is(":checked") == true) {
        ShowAll = "1";
    }
    ...
    if ($("[id$=hdfIsPR_SCinBothSBU]").val() == "0") {
        var sbuID = $("[id$=PRH_SO_BIZUNIT]").val();
        if (sbuID != "0") {
            GrandScriptUtils.MakeAutoComplete("txtSO", PurchaseRequestConfig.FillIODropdownURL + sbuID + "&PrhPK=" + $("[id$=PRH_PK]").val() + "&SohPK=0" + "&isGlove=" + isGlove + "&ShowAll=" + ShowAll + "&IsDispatched=" + IsDispatched, "PRH_SO_HDR", true, false, "BizUnitPk", true);
        }
        else {
            GrandScriptUtils.MakeAutoComplete("txtSO", PurchaseRequestConfig.FillIODropdownURL + $("[id$=BizUnitPk]").val() + "&PrhPK=" + $("[id$=PRH_PK]").val() + "&SohPK=0" + "&DeptPk=" + $("[id$=hdfDeptID]").val() + "&isGlove=" + isGlove + "&ShowAll=" + ShowAll + "&IsDispatched=" + IsDispatched, "PRH_SO_HDR", true, false, "BizUnitPk", true);
        }
    }
    else {

        GrandScriptUtils.MakeAutoComplete("txtSO", PurchaseRequestConfig.FillIODropdownURLAllSBU + "&PrhPK=" + $("[id$=PRH_PK]").val() + "&SohPK=0" + "&isGlove=" + isGlove + "&ShowAll=" + ShowAll + "&IsDispatched=" + IsDispatched, "PRH_SO_HDR", true, false, "BizUnitPk", true);
    }
}
...
function AddValidations(mode) {
    ...
    if (mode == 1) {
        ...
        if ($("[id$=hdfProjectRequired]").val() == "1") {
            $("[id$=PRH_INVESTOR]").rules("add", {
                selectNone: true,
                messages: { selectNone: "Translate(SelectProject)" }
            });
        }
...
function FillCategoryMaterials(catgID) {
    ...
    var autoURL = PurchaseRequestConfig.MaterialURL + "&StockExist=0";
    if (getQueryStringValue("Modify").toLowerCase() == "1")
        autoURL = PurchaseRequestConfig.MaterialURL + "&Alt=1&ItemPK=" + $("[id$=hdfEditedMatPK]").val();

    GrandScriptUtils.MakeAutoCompleteLimitLen("ItemCodeMaterial", autoURL, "MaterialPK", true, false, "MaterialCategoryPK", true, "Store", "", "", $("[id$=AutoStartValue]").val());
...
function FillIONumber(selectVal) {
    ...
    if ($("[id$=hdfIsPR_SCinBothSBU]").val() == "0") {
        var sbuID = $("[id$=PRH_SO_BIZUNIT]").val();
        if (sbuID != "0") {
            GrandScriptUtils.MakeAutoComplete("txtSO", PurchaseRequestConfig.FillIODropdownURL + sbuID + "&PrhPK=" + $("[id$=PRH_PK]").val() + "&SohPK=" + selectVal + "&isGlove=" + isGlove + "&IsDispatched=" + IsDispatched, "PRH_SO_HDR", true, false, "BizUnitPk", true);
        }
        else {
            GrandScriptUtils.MakeAutoComplete("txtSO", PurchaseRequestConfig.FillIODropdownURL + $("[id$=BizUnitPk]").val() + "&PrhPK=" + $("[id$=PRH_PK]").val() + "&SohPK=" + selectVal + "&DeptPk=" + $("[id$=hdfDeptID]").val() + "&isGlove=" + isGlove + "&IsDispatched=" + IsDispatched, "PRH_SO_HDR", true, false, "BizUnitPk", true);
        }
    }
    else {

        GrandScriptUtils.MakeAutoComplete("txtSO", PurchaseRequestConfig.FillIODropdownURLAllSBU + "&PrhPK=" + $("[id$=PRH_PK]").val() + "&SohPK=" + selectVal + "&isGlove=" + isGlove + "&IsDispatched=" + IsDispatched, "PRH_SO_HDR", true, false, "BizUnitPk", true);
    }
    if (storeLoaded) {
        BindGrid();
    }
    ...
    $.get(PurchaseRequestConfig.GetMaterialDetails + $("[id$=BizUnitPk]").val() + "&MaterialID=" + materialID, function (data) {
        if (data) {
            if (materialID != 0) {
                ...
                $("[id$=hdfMaxOrderQty]").val(data[0].ITM_MAX_OQ);
...
function BindGrid() {
    ...
    if ($("[id$=PRH_SO_HDR]").val() == "0" || $("[id$=PRH_SO_HDR]").val() == null) {
        ...
        if ($("[id$=hdfEnableGlovePR]").val() == "1") {
            if ($("[id$=chkIsGloveRequest]").is(":checked") == true) {
                ajaxUrl = PurchaseRequestConfig.CustomerItemRequestGetGridURL + "&SohPK=" + $("[id$=PRH_SO_HDR]").val() + "&Dept=" + $("[id$=Store]").val() + "&Type=" + $("[id$=IPD_TYPE]").val() + "&RCount=" + $("[id$=hdfPRRowCount]").val() + "&ItmCat=" + $("[id$=hdfItemCategoryPK]").val();
            }
            else {
                ajaxUrl = PurchaseRequestConfig.PackingMaterialBindGridURL + "&SohPK=" + $("[id$=PRH_SO_HDR]").val() + "&Dept=" + $("[id$=Store]").val() + "&Type=" + $("[id$=IPD_TYPE]").val() + "&RCount=" + $("[id$=hdfPRRowCount]").val() + "&ItmCat=" + $("[id$=hdfItemCategoryPK]").val() + "&ItmCatSC=" + $("[id$=hdfDataFromSC]").val();
            }
        }
        else {
            ajaxUrl = PurchaseRequestConfig.PackingMaterialBindGridURL + "&SohPK=" + $("[id$=PRH_SO_HDR]").val() + "&Dept=" + $("[id$=Store]").val() + "&Type=" + $("[id$=IPD_TYPE]").val() + "&RCount=" + $("[id$=hdfPRRowCount]").val() + "&ItmCat=" + $("[id$=hdfItemCategoryPK]").val() + "&ItmCatSC=" + $("[id$=hdfDataFromSC]").val();
    ...
    $.get(PurchaseRequestConfig.DepartmentsDDL + PK + "&ProcessID=" + $("[id$=hdfProcessID]").val() + "&RefID=" + $("[id$=hdfRefID]").val(), function (data) {
        if ($("[id$=hdfIsWkfSettingPostBackReqd]").val() == "1" && $("[id$=PRH_PK]").val() == 0 && data != null && data.length == 1) {
            ...
            if (data != null && data.length == 1) {
                ...
                if (PK == 0 && $("[id$=hdfReqDeptPk]").val() != "0")
                    PK = $("[id$=hdfReqDeptPk]").val();
    ...
    if (ParentPK > 0) {
        ...
        $.get(PurchaseRequestConfig.SubDepartmentsDDL + PK + "&ParentDeptPk=" + ParentPK, function (data) {
            if (data != null && data.length == 1) { //If the Request by Dept. ddl contain only one value,Select it by default
                GrandScriptUtils.FillDropDown(drpID, data, true, false, data[0].Value);
                SubDeptChange($("[id$=hdfCostCenter]").val());
            }
            else {
                GrandScriptUtils.FillDropDown(drpID, data, true, true, PK);
                if (PK == 0)//If New Mode
...
function SubDeptChange(costcenterPK) {
    if (costcenterPK == '' || costcenterPK == null) costcenterPK = 0;
    FillCostCenter($("[id$=PRH_ISSUE_SUB_DEPT]").val(), costcenterPK);
    return false;
}
function FillCostCenter(IssueSubDeptPK, PK) {
    //<summary>function To Fill Category Details </summary>
    // Get id of the Category DropDown
    var drpID = $("select[id$=PRH_COST_CENTER]").attr('id');
    if (IssueSubDeptPK == null || IssueSubDeptPK == undefined)
        IssueSubDeptPK = $("[id$=PRH_ISSUE_SUB_DEPT]").val();

    if (PK == null || PK == undefined)
        PK = 0;
    //    if ( $("select[id$=PRH_ISSUE_SUB_DEPT]").val() != '' && $("select[id$=PRH_ISSUE_SUB_DEPT]").val() != '-1' && $("select[id$=PRH_ISSUE_SUB_DEPT]").val() != '0')
    //        SubDeptPK = $("select[id$=PRH_ISSUE_SUB_DEPT]").val();
    if (IssueSubDeptPK > 0) {
        //Fill Category Details to the Category DropDown, Name as Text, PK as Value
        $.get(PurchaseRequestConfig.GetCostCenterURL + IssueSubDeptPK + "&BizUnit=" + PurchaseRequestConfig.BizUnitPk + "&CnmPK=" + PK + "&active=1", function (data) {
            if (data != null && data.length == 1) { //If the Request by Dept. ddl contain only one value,Select it by default
                GrandScriptUtils.FillDropDown(drpID, data, true, false, data[0].Value);
            }
            else {
...
function ReqDeptChange() {
    ...
    //FillCostCenter($("select[id$=PRH_ISSUE_SUB_DEPT]").val(), 0);
}
...
function FillPurchaseRequestDetails(purchaseReqObj) {
    ...
    if (purchaseReqObj.MPRH_PMH_PK != null && purchaseReqObj.MPRH_PMH_PK != "undefined") {
        $("[id$=MPRH_PMH_PK]").val(purchaseReqObj.MPRH_PMH_PK);
        FillIssuingDepartment(0);
    }
    else
        if (purchaseReqObj.PRH_PMH_PK != null && purchaseReqObj.PRH_PMH_PK != "undefined") {
            $("[id$=PRH_PMH_PK]").val(purchaseReqObj.PRH_PMH_PK);
            FillIssuingDepartment(0);
        }
        else
            FillIssuingDepartment(purchaseReqObj.PRH_ISSUE_DEPT, purchaseReqObj.PRH_ISSUE_SUB_DEPT);
    $("[id$=hdfCostCenter]").val(purchaseReqObj.PRH_COST_CENTER);
    ...
    if (purchaseReqObj.PRH_IS_GLOVE == 1)
        $("[id$=chkIsGloveRequest]").attr('checked', true);
    else
        $("[id$=chkIsGloveRequest]").attr('checked', false);

    $("[id$=PRH_IS_COMPLETED]").val(purchaseReqObj.PRH_IS_COMPLETED);

    if (purchaseReqObj.PRH_IS_COMPLETED == 1) {
        $("[id$=chkPRCompleted]").attr('checked', true);
    }
    else {
        $("[id$=chkPRCompleted]").attr('checked', false);
    }
...
function AddPurchaseRequest(isValidated) {
    ...
    if ($(document.forms[0]).valid()) {
        ...
        if (!isValidated) {
            ...
            if ($("[id$=hdfMaxOrderQtyEnabled]").val() == "1" && parseFloat($("[id$=hdfMaxOrderQty]").val()) > 0) {

                var materialPK = $("[id$=MaterialPK]").val();
                var SlNo = $("[id$=hdfSlNo]").val()
                var TotoalPRQty = 0;
                var CurrentPRQty = parseFloat($("[id$=PurchaseQty]").val());

                var newList = PurchaseRequestConfig.PurchaseRequestList.filter(v => v.PRD_ITEM == materialPK && v.SLNO != SlNo)
                $(newList).each(function (i) {
                    TotoalPRQty += parseFloat(newList[i].PRD_QTY_REQUESTED);
                })

                TotoalPRQty = parseFloat(TotoalPRQty);
                if (parseFloat(CurrentPRQty + TotoalPRQty) > parseFloat($("[id$=hdfMaxOrderQty]").val())) {
                    GrandScriptUtils.ShowModal(PurchaseRequestConfig.Err_MaxOrderQty, PurchaseRequestConfig.MessageBoxTitle);
                    return false;
                }
            }
        ...
        PurchaseRequestConfig.PurchaseRequestObj.ITM_MAX_OQ = $("[id$=hdfMaxOrderQty]").val();
        ...
        if (parseInt(PurchaseRequestConfig.PRDITEM) == 0) {
            ...
            for (var i in PurchaseRequestConfig.PurchaseRequestList) {
                ...
                if (isItemExists) {
                    if (getQueryStringValue("Modify").toLowerCase() == "1")
                        PurchaseRequestConfig.PurchaseRequestObj.PRD_PK = PurchaseRequestConfig.PurchaseRequestList[i].PRD_PK;
...
function IsSCPRCompleted() {
    SCQtyTotal = 0.0;
    SCPrevReqQtyTotal = 0.0;
    SCReqQtyTotal = 0.0;
    PurchaseRequestConfig.PurchaseRequestList = $("#divData").data("PurchaseRequestData");
    for (var i in PurchaseRequestConfig.PurchaseRequestList) {
        SCReqQtyTotal = SCReqQtyTotal + parseFloat(PurchaseRequestConfig.PurchaseRequestList[i].PRD_QTY_REQUESTED);
    }

    $("#grdPuchasePendingList tr:has(td)").each(function (index) {
        var itemPK = GrandGrid.Utilities.GetColumnValue($(this), "ITM_PK", "grdPuchasePendingList");
        var SOQty = GrandGrid.Utilities.GetColumnValue($(this), "SOD_SO_QTY", "grdPuchasePendingList");
        var PRDQtyApproved = GrandGrid.Utilities.GetColumnValue($(this), "PRD_QTY_APPROVED", "grdPuchasePendingList");
        SCQtyTotal = SCQtyTotal + parseFloat(SOQty);
        SCPrevReqQtyTotal = SCPrevReqQtyTotal + parseFloat(PRDQtyApproved);
    });
    return SCReqQtyTotal >= (SCQtyTotal - SCPrevReqQtyTotal);
}
//____________
    ...
    if (!status || PurchaseRequestConfig.ExistingItems.length == 0) {
        ...
        $("#grdPuchasePendingList tr input[type=checkbox]:checked").each(function () {
            ...
            if ($(tr[0]).parent().find("th").length == 0) {
                ...
                PurchaseRequestConfig.PurchaseRequestObj.UOM = GrandGrid.Utilities.GetColumnValue(tr, PurchaseRequestConfig.UOM_CODE, grdID);
                PurchaseRequestConfig.PurchaseRequestObj.PRD_QTY_REQUESTED = $(tr).find("td:eq(" + colIndex + ") input[type=text]").val();
                if ($("[id$=chkIsGloveRequest]").is(":checked") == true && $("[id$=hdfEnableGlovePR]").val() == "1") {
                    PurchaseRequestConfig.PurchaseRequestObj.PRD_PURPOSE = GrandGrid.Utilities.GetColumnValue(tr, "SOH_REFERENCE", grdID);
                }
                else {
                    PurchaseRequestConfig.PurchaseRequestObj.PRD_PURPOSE = "";
                }
...
function DisableFileds(catVal) {
    ...
    $("select[id$=PRH_ISSUE_SUB_DEPT]").attr("disabled", true);
    $("[id$=PRH_USER]").attr("disabled", "disabled");
    $("select[id$=PRH_COST_CENTER]").attr("disabled", "disabled");
    if (catVal == "1")
        $("select[id$=PRH_PO_CATEGORY]").attr("disabled", true);
    $("[id$=chkIsGloveRequest]").attr("disabled", "disabled");
    $("[id$=chkPRCompleted]").attr("disabled", "disabled");
}
...
function EnableFileds() {
    ...
    $("select[id$=PRH_COST_CENTER]").removeAttr("disabled");

    $("[id$=chkIsGloveRequest]").removeAttr("disabled");
    $("[id$=chkPRCompleted]").removeAttr("disabled");

}
function CheckChangePRComplete() {
    if ($("[id$=chkPRCompleted]").is(":checked") == true) {
        GrandScriptUtils.ShowModal(PurchaseRequestConfig.ThisSCWillNotShow.fontcolor("red"), PurchaseRequestConfig.MessageBoxTitle);
    }
    return false;
}
        ...
        if ($("[id$=hdfDeptID]").val() != data) {
            ...
            if ($(document.forms[0]).valid()) {
                ...
                if (PurchaseRequestConfig.PurchaseRequestList.length > 0) {
                    ...
                    if ($("[id$=chkIsGloveRequest]").is(":checked") == true && $("[id$=hdfEnableGlovePR]").val() == "1") {
                        if ($("[id$=PRH_SO_HDR]").val() != "0" && $("[id$=PRH_SO_HDR]").val() != null) {
                            $("[id$=PRH_IS_GLOVE]").val('1');
                        }
                        else {
                            $("[id$=PRH_IS_GLOVE]").val('0');
                        }
                    }
                    else {
                        $("[id$=PRH_IS_GLOVE]").val('0');
                    }
                    //                    if ($("[id$=chkPRCompleted]").is(":checked") == false) {
                    //                        if ($("[id$=PRH_IS_COMPLETED]").val() == "0") {
                    if ($("[id$=hdfStatus]").val() == "0" && command != "Draft") {
                        if (IsSCPRCompleted())
                            $("[id$=PRH_IS_COMPLETED]").val('1');
                        else {
                            $("[id$=PRH_IS_COMPLETED]").val('0');
                        }
                    }
                    //                        }
                    //                    }
                    if ($("[id$=chkPRCompleted]").is(":checked") == true && command != "Draft") {
                        $("[id$=PRH_IS_COMPLETED]").val('1');
                    }
                    ...
                    $.post(PurchaseRequestConfig.SavePurchaseRequest, jSonString, function (data) {
                        ...
                        else if (parseInt(data[0]) == -67) {
                            //                    $(this).fadeOut("slow").remove();
                            GrandScriptUtils.ShowModal(PurchaseRequestConfig.PRReqQtySCExceed, PurchaseRequestConfig.MessageBoxTitle);
                            $("[id$=SubmitFlag]").val('0')
                        }
                        else if (parseInt(data[0]) == -68) {
                            //                    $(this).fadeOut("slow").remove();
                            GrandScriptUtils.ShowModal(PurchaseRequestConfig.POExist, PurchaseRequestConfig.MessageBoxTitle);
                            $("[id$=SubmitFlag]").val('0')
                        }
                        else if (parseInt(data[0]) == -69) {
                            //Max order qty exeeded
                            GrandScriptUtils.ShowModal(PurchaseRequestConfig.Err_MaxOrderQty, PurchaseRequestConfig.MessageBoxTitle);
                            $("[id$=SubmitFlag]").val('0')
                        }
                        else if (parseInt(data[0]) == -70) {
                            $("[id$=SubmitFlag]").val('0')
...
function ShowBudgetValidation() {
    var msgTitle;
    var msg;
    msgTitle = PurchaseRequestConfig.Confirmation;
    msg = PurchaseRequestConfig.BudgetValidationMsg;
...
function ClearPurchaseRequest() {
    ...
    $("[id$=hdfMaxOrderQty]").val("0");
    $("[id$=hdfEditedMatPK]").val("0");//added with MaxOQ
...
function FillPurchaseRequest(tr) {
    ...
    $("[id$=hdfEditedMatPK]").val(PurchaseRequestConfig.PRDITEM);
    $("[id$=hdfMaxOrderQty]").val(GrandGrid.Utilities.GetColumnValue(tr, PurchaseRequestConfig.ITM_MAX_OQ, grdID));
    FillCategoryMaterials();
    $("[id$=ItemCodeMaterial]").val(GrandGrid.Utilities.GetColumnValue(tr, PurchaseRequestConfig.ITM_TEXT, grdID));
    FillUOM($("[id$=hdfEditedMatPK]").val(), GrandGrid.Utilities.GetColumnValue(tr, PurchaseRequestConfig.PRD_UOM, grdID));
    FillItemUOM($("[id$=MaterialPK]").val(), GrandGrid.Utilities.GetColumnValue(tr, PurchaseRequestConfig.PRD_UOM, grdID));
    $("[id$=MaterialPK]").val($("[id$=hdfEditedMatPK]").val());
...
function AfterGridBind(grdID) {
    ...
    var modifyVal = getQueryStringValue("Modify").toLowerCase();
    if (modifyVal == "1") {
        $("#grdPurchaseRequest th:last").show();
        $("#grdPurchaseRequest th:last").find("id$=imbEdit").show();
        $("#grdPurchaseRequest th:last").find("id$=imbDelete").hide();
        $("#grdPurchaseRequest tr:has(td)").each(function (index) {
            if (isViewMode) {
                $(this).find("td:last").show();
                //$(this).find("td:last").find("id$=imbEdit").show();
                //$(this).find("td:last").find("id$=imbDelete").hide();
            }
        });
    }
}

function ShowEditForModify() {
    if ($("[id$=hdfIsPRModify]").val() == "1") {
        $("#grdPurchaseRequest th:last").show();
        $("#grdPurchaseRequest th:last").find("id$=imbEdit").show();
        $("#grdPurchaseRequest th:last").find("id$=imbDelete").hide();
    }
...
function FillInvestor(SelectVal) {
    ///<summary>function used to Fill investor</summary
    var active = SelectVal > 0 ? 2 : 1;
    var drpID = $("select[id$=PRH_INVESTOR]").attr("id");
    $.get(PurchaseRequestConfig.GetInvestmentValue + SelectVal + "&Active=" + active, function (data) {
        GrandScriptUtils.FillDropDown(drpID, data, true, true, SelectVal);
        //$("select[id$=PRH_PO_CATEGORY]").attr("disabled", true);
    });
}
