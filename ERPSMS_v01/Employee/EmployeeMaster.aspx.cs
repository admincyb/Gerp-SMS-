...
{
    public partial class EmployeeMaster : ERP.Store.UI.MyBasePage
    {
        ...
        private int EmployeeLimit
        {
            get
            {
                return this.ViewState["EmployeeLimit"] == null ? TotalUsersCount + 1 : (int)(this.ViewState["EmployeeLimit"]);
            }
            set
            {
                this.ViewState["EmployeeLimit"] = value;
            }
        }
        /// <summary>
        /// Current Status
        ...
        private XacEmpType xacEmpTypeObj;
        private List<XacEmpType> xacEmpTypeLst;
            ...
            try
            {
                ...
                switch (type)
                {
                    ...
                    case ControlsEnum.EMPLOYEETYPE:
                        xacEmpTypeObj = CommonFunctions.Initilize<XacEmpType>();
                        xacEmpTypeObj.xetActive = Convert.ToByte(DbActiveStatus.ACTIVE);
                        xacEmpTypeObj.xetSequence = 0;
                        xacEmpTypeLst = employeeServiceClient.GetXacEmpType(xacEmpTypeObj);
                        break;
        ...
        {
            DataTable dtConfig = CommonBL.GetApplicaitonConfiguaration("USER MODULE SETTING", string.Empty);
            if (dtConfig != null && dtConfig.Rows.Count > 0)
            {
                ...
                {
                    if (retVal.Length > 0)
                        EmployeeLimit = Convert.ToInt32((retVal.Split('-')[(int)UserModuleLimitIndex.Employee]));
                }
                catch (Exception ex)
                {
                }
        ...
        private string CreateRoleMappingXml()
        {
            string resultXml = string.Empty;
            if (RoleMappingHeaderViewState.RoleList == null)
            {
                return resultXml;
            }
            foreach (var item in RoleMappingHeaderViewState.RoleList)
            ...
            foreach (TreeNode node in trvEmpRole.Nodes)
            {
                if (node.Checked)
                {
                    int pk = GetNullableInt(node.Value).Value;
                    RoleMapping roleMapping = RoleMappingHeaderViewState.RoleList
                        .Where(x => x.ermRole == pk)
                        .Single();
                    roleMapping.IS_CHECKED = "true";
                    roleMapping.ermActive = 1;
                }
            }
            RoleMappingHeader header = new RoleMappingHeader();
            header.EMP_PK = RoleMappingHeaderViewState.RoleList[0].ERM_EMP;
            header.RoleList = RoleMappingHeaderViewState.RoleList
                                    .Where(x => x.IS_CHECKED == "true")
                                    .ToList();
            resultXml = CommonFunctions.XmlSerialize<RoleMappingHeader>(header);
            return resultXml;
        }
        ...
        private void ActivateEmployeeRole()
        {
            spnEmployeeRoles.Attributes["class"] = "tab-active";
            spnEmployeeList.Attributes["class"] = "tab-inactive";
            lbnEmployeeRoles.CssClass = "tab-active";
            lbnEmployee.CssClass = "tab-inactive";
            divRoleMapping.Visible = true;
            PageAction_Entry.Visible = false;
        }
