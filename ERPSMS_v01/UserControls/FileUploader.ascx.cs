...
namespace ERPSMS_v01.UserControls
{
    public partial class FileUploader : System.Web.UI.UserControl
    {
        ...
        public delegate void AfterFileDelete(object sender, EventArgs e);
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (current != null)
            {
                current.RegisterPostBackControl(btnUploadFile);
            }
        ...
        public void UploadFile(FileCollectionEventArgs e)
        {
            ...
            if (e.HasFiles && oHttpFileCollection.Count > 0 && oHttpFileCollection[fileKey].FileName != string.Empty)
            {
                ...
                string fileTitle = txtFlpFileTite.Text.Trim();
                ...
                if (fileStatus != "Invalid")
                {
                    if (CheckFolderExists(uploadPath))
                    {
                        ...
                        grdUploadedFiles.DataSource = Upload;
                    ...
                    {
                        if (CreateFolder(Request.MapPath("~/"), "Upload"))
                        {
                            filePath = uploadPath + "//" + actName + ext;
                            oHttpPostedFile.SaveAs(filePath);
                            fileUpload = new AttachmentBO();
        ...
        public List<AttachmentBO> GetAttachedFiles()
        {
            ...
            foreach (GridViewRow row in grdUploadedFiles.Rows)
            {
            ...
            if (Upload != null && Upload.Count == titleList.Count)
        ...
        public void SetAttachedFiles(List<AttachmentBO> lstAttachment)
        {
            ...
            grdUploadedFiles.DataSource = lstAttachment;
            grdUploadedFiles.DataBind();
