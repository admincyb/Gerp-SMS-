...
namespace ERPSMS_v01.UserControls
{
    public partial class MenuControl : System.Web.UI.UserControl
    {
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                string SessionDept = Session[SessionStrings.CurDept] != null ? Session[SessionStrings.CurDept].ToString() : "0";
                if (Request.QueryString["Dep"] != null)
                    if (Request.QueryString["MnChange"] != null
                        && Request.QueryString["MnChange"] == "1"
                        || (currentUser.CurrentDeptPK.ToString() != Request.QueryString["Dep"] || (SessionDept != Request.QueryString["Dep"]))
                       )
                    {
                        Session[SessionStrings.CurDept] = Convert.ToInt32(Request.QueryString["Dep"].ToString());
                        currentUser.CurrentDeptPK = Convert.ToInt32(Request.QueryString["Dep"].ToString());
        ...
        protected void ActionHandler(object sender, EventArgs e)
        {
            ...
            switch (commonActions)
            {
                case ActionsEnum.LINKBUTTONCLICK:
                    string url = (sender as LinkButton).CommandArgument;
                    if (url.Contains(Resources.PageURL.COA))
                    {
                        ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "Openwindow", "OpenPDF('" + url + "');", true);
                    }
                    else if (url.Contains(Resources.PageURL.UploadTemplate))
                    {
                        ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ClosePagePopup", "$('#updateProgress').hide();", true);
                        ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "OpenDocs", "OpenUploadDocs('" + url + "');", true);
                        break;
                    }
        ...
        protected void ddlLocation_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //ScriptManager.RegisterStartupScript(this.Parent.Page, typeof(Page), "ShowMenu1", "$(document).ready(function () {ShowMenu();});", true);
        }
        ...
        protected void ddlCostCenter_SelectedIndexChanged(object sender, EventArgs e)
        {
            IsIndexChanged = true;
            FillMenu(true);
            //if (System.Configuration.ConfigurationManager.AppSettings["SMSURL"] != null
            //    || System.Configuration.ConfigurationManager.AppSettings["PORTALURL"] != null
            //    || System.Configuration.ConfigurationManager.AppSettings["ASSETURL"] != null
            //    || System.Configuration.ConfigurationManager.AppSettings["PRODUCTIONURL"] != null
            //    || System.Configuration.ConfigurationManager.AppSettings["CONSTRUCTIONURL"] != null)
            //{
            if (System.Configuration.ConfigurationManager.AppSettings["ERP"] != null
                && System.Configuration.ConfigurationManager.AppSettings["ERP"].ToString().ToLower() == "1")
            {
                string pageServer = string.Empty;
                string virtualDirectory = string.Empty;
                ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ClosePopup", "ClosePopup();", true);
                DataTable dtUrl = CommonBL.GetDepartmentUrl(Convert.ToInt32(Session[SessionStrings.CurDept].ToString()));
                if (!string.IsNullOrEmpty(dtUrl.Rows[0]["DPT_PAGE_SERVER"].ToString()))
                {
                    pageServer = dtUrl.Rows[0]["DPT_PAGE_SERVER"].ToString().Replace('/', ' ');
                    virtualDirectory = System.Configuration.ConfigurationManager.AppSettings["VirtualDirectory"].ToString().Replace('/', ' ');
                    if (pageServer.Trim().ToLower() != virtualDirectory.Trim().ToLower())
                    {
                        Response.Redirect(dtUrl.Rows[0]["DPT_PAGE_SERVER"].ToString() + dtUrl.Rows[0]["DPT_PAGE_URL"].ToString() + "?Dep=" + Session[SessionStrings.CurDept].ToString() + "&MnChange=1", true);
                    }
                }
            }
        ...
        /// <summary>
        /// Fill Department Biz Unit
        /// </summary>
        /// <param name="dept"></param>
        private void GetDepartmetBizUnit(int dept)
        {
            bool IsChangeBiz = false;
            DataTable dtDept;
            dtDept = CommonBL.GetDepartmentDetails(dept);
            if (dtDept != null && dtDept.Rows.Count > 0)
            {
                if (currentUser.SBUID != Convert.ToInt32(dtDept.Rows[0]["DPT_BIZUNIT"]))
                    IsChangeBiz = true;
                currentUser.SBUID = Convert.ToInt32(dtDept.Rows[0]["DPT_BIZUNIT"]);
                //  this.ddlLocation.SelectedValue = currentUser.SBUID.ToString();
                BusinessObject.SBU sbu = new BusinessObject.SBU() { CurrentSBU = "", CurrentSBUPK = Convert.ToInt32(dtDept.Rows[0]["DPT_BIZUNIT"]) };
                BusinessObject.Department deptBiz = new BusinessObject.Department() { CurrentSBU = "", CurrentSBUPK = currentUser.SBUID, CurrentDept = dtDept.Rows[0]["DPT_NAME"].ToString(), CurrentDeptPK = currentUser.CurrentDeptPK };
                BusinessLogic.AccountManagement.UserAuthBL userAuth = new BusinessLogic.AccountManagement.UserAuthBL();
                userAuth.SetUserProperty(UserPropertyEnum.SBUDept, sbu, deptBiz);
                if (IsChangeBiz)
                    FillDepartment(currentUser.PKUser, currentUser.SBUID, false);
            }
        }
        /// <summary>
        /// Fill Location Dropdown based on UserPK and SBU
        /// </summary>
        public void FillLocation(bool isEditUser, bool? fromLoad = false)
        {
            DataTable sbuDetails;

            sbuDetails = BusinessLogic.CommonManagement.CommonBL.GetBizUnit(currentUser.PKUser, currentUser.SBUID);
            if (sbuDetails.Rows.Count > 0)
            {
                ...
                if (currentUser.SBUID != 0)
                    this.ddlLocation.SelectedValue = currentUser.SBUID.ToString();
                if (isEditUser == true && fromLoad == false)
                {
        ...
        private void FillDepartment(int userPK, int sbuID, bool isEditUser)
        {
            ...
            this.ddlCostCenter.SelectedIndex = -1;
            Int16 module;
            Int16.TryParse(ConfigurationManager.AppSettings[ERP.Utilities.ConfigStrings.GERPModule], out module);
            dtDept = BusinessLogic.CommonManagement.CommonBL.GetDepartment(userPK, sbuID, module);
            ...
            if (dtDept.Rows.Count > 0)
            {
                try
                {
                    DataRow curRow = dtDept.AsEnumerable().SingleOrDefault(row => row.Field<int>("DPT_PK") == currentUser.CurrentDeptPK);
                    if (currentUser.CurrentDeptPK != 0 && curRow != null)
                        this.ddlCostCenter.SelectedValue = Session[SessionStrings.CurDept] != null ? Session[SessionStrings.CurDept].ToString() : currentUser.CurrentDeptPK.ToString();
                }
                catch
                {
                }
        ...
        /// </summary>
        public void FillMenu(bool isEditUser)
        {
            if (isEditUser)
            {
                ...
                Literal litDept = (Literal)this.Page.Master.FindControl("litDept");
                litDept.Text = ddlCostCenter.SelectedItem.Text;
                if (Session[SessionStrings.CurDept] != null && !IsIndexChanged)
                {
                    ddlCostCenter.SelectedIndex = ddlCostCenter.Items.IndexOf(ddlCostCenter.Items.FindByValue(Session[SessionStrings.CurDept].ToString()));
                }
