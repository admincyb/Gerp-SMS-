<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="MenuControl1.ascx.cs" Inherits="ERPSMS_v01.UserControls.MenuControl1" %>
    ...
    function SetAutoCompleteMenu() {
        ...
        $("[id$=txtMenuSearchBy]").autocomplete({
            ...
            select: function (event, ui) {
                ...
                //window.location = pageURL.replace(location.pathname, virtualPath == "" ? selecteditem.id : virtualPath + "/" + selecteditem.id);
                ...
                <td align="center">
                    <ul class="regfloat">
                        <li>
                            ...
                            <asp:DropDownList runat="server" ID="ddlLocation" AutoPostBack="True" OnSelectedIndexChanged="ddlLocation_SelectedIndexChanged">
                            </asp:DropDownList>
                        </li>
                        <li>
                            <asp:Label runat="server" ID="lblCostCenter" Text="<%$Resources:Controls,Department %>"
                                AssociatedControlID="ddlCostCenter"></asp:Label>
                            <asp:DropDownList runat="server" ID="ddlCostCenter" AutoPostBack="True" OnSelectedIndexChanged="ddlCostCenter_SelectedIndexChanged">
                            </asp:DropDownList>
                        </li>
                        <li>
                                                                                ...
                                                                                <asp:Repeater runat="server" ID="rptLinks">
                                                                                    <ItemTemplate>
                                                                                        <asp:LinkButton runat="server" ID="lbnLink" CommandArgument='<%#ConfigurationManager.AppSettings["VirtualDirectory"].ToLower() != string.Empty ? DataBinder.Eval(Container.DataItem, "PostUrl").ToString()+DataBinder.Eval(Container.DataItem, "LinkUrl").ToString():DataBinder.Eval(Container.DataItem, "LinkUrl") %>'
                                                                                            Text='<%#DataBinder.Eval(Container.DataItem, "LinkText")%>' CommandName="LINKBUTTONCLICK" OnClick="ActionHandler"></asp:LinkButton>
                                                                                    </ItemTemplate>
                                                                                </asp:Repeater>
                                                                            </ItemTemplate>
                                                                        </asp:Repeater>
