<%@ Page Title="" Language="C#" MasterPageFile="~/ERPSMS_2.Master" AutoEventWireup="true"
    Theme="Classic" CodeBehind="CustomerRegistration.aspx.cs" Inherits="ERPSMS_v01.OrderToCash.CustomerRegistration" %>
    ...
    <script type="text/javascript">
        ...
        function FixCustomizedProductAttributesWidth() {
            ...
            if (field.getBoundingClientRect) {
                ...
                var availableWidth = Math.max(350, containerRect.right - fieldRect.left - 160);
        ...
        function MoveCustomizedPackingSpecsBelowPackSpecName() {
            var packSpecName = document.querySelector('input[id$="CIM_PACKING_SPEC_NAME"]');
            var customizedSpec = document.querySelector('textarea[id$="CIM_PACKING_SPEC_DESC"], input[id$="CIM_PACKING_SPEC_DESC"]');
            if (!packSpecName || !customizedSpec) {
                return;
            }

            var targetRow = packSpecName.closest ? packSpecName.closest('tr') : null;
            var specRow = customizedSpec.closest ? customizedSpec.closest('tr') : null;
            if (targetRow && specRow && targetRow.parentNode && specRow !== targetRow && targetRow.nextSibling !== specRow) {
                targetRow.parentNode.insertBefore(specRow, targetRow.nextSibling);
            }

            var specCell = customizedSpec.closest ? customizedSpec.closest('td') : null;
            if (specCell) {
                specCell.colSpan = Math.max(specCell.colSpan || 1, 2);
                specCell.style.setProperty('display', 'block', 'important');
                specCell.style.setProperty('width', '100%', 'important');
                specCell.style.setProperty('box-sizing', 'border-box', 'important');
            }

            if (specRow) {
                specRow.style.setProperty('display', 'block', 'important');
                specRow.style.setProperty('width', '100%', 'important');
            }

            var specWrapper = customizedSpec.closest ? customizedSpec.closest('.div2col-M, .divcol-M0, .divcol-M') : null;
            specWrapper = specWrapper || customizedSpec.parentElement;
            if (specWrapper) {
                specWrapper.style.setProperty('display', 'grid', 'important');
                specWrapper.style.setProperty('grid-template-columns', '240px minmax(0, 1fr)', 'important');
                specWrapper.style.setProperty('column-gap', '10px', 'important');
                specWrapper.style.setProperty('align-items', 'center', 'important');
                specWrapper.style.setProperty('width', '100%', 'important');
                specWrapper.style.setProperty('box-sizing', 'border-box', 'important');

                var specLabel = specWrapper.querySelector('label, span');
                if (specLabel) {
                    specLabel.style.setProperty('display', 'block', 'important');
                    specLabel.style.setProperty('width', '240px', 'important');
                    specLabel.style.setProperty('min-width', '240px', 'important');
                    specLabel.style.setProperty('max-width', '240px', 'important');
                    specLabel.style.setProperty('padding-right', '4px', 'important');
                    specLabel.style.setProperty('text-align', 'right', 'important');
                    specLabel.style.setProperty('white-space', 'nowrap', 'important');
                    specLabel.style.setProperty('box-sizing', 'border-box', 'important');
                }
            }

            var description = document.querySelector('textarea[id$="CIM_DESC"], input[id$="CIM_DESC"]');
            var descriptionWidth = description ? description.getBoundingClientRect().width : 0;
            customizedSpec.style.setProperty('display', 'block', 'important');
            customizedSpec.style.setProperty('width', descriptionWidth > 0 ? descriptionWidth + 'px' : 'min(75%, calc(100% - 260px))', 'important');
            customizedSpec.style.setProperty('min-width', '0', 'important');
            customizedSpec.style.setProperty('max-width', 'none', 'important');
            customizedSpec.style.setProperty('box-sizing', 'border-box', 'important');
        }

        if (window.$) {
            $(document).ready(FixCustomizedProductAttributesWidth);
            $(document).ready(AlignTermsActiveBelowDescription);
            $(document).ready(MoveCustomizedPackingSpecsBelowPackSpecName);
        }
        if (document.readyState === 'loading') {
            ...
            document.addEventListener('DOMContentLoaded', MoveCustomizedPackingSpecsBelowPackSpecName);
        } else {
            FixCustomizedProductAttributesWidth();
            AlignTermsActiveBelowDescription();
            MoveCustomizedPackingSpecsBelowPackSpecName();
        }
        ...
        window.addEventListener('load', MoveCustomizedPackingSpecsBelowPackSpecName);
        ...
        window.setTimeout(MoveCustomizedPackingSpecsBelowPackSpecName, 0);
        window.setTimeout(MoveCustomizedPackingSpecsBelowPackSpecName, 500);
        window.setTimeout(MoveCustomizedPackingSpecsBelowPackSpecName, 1500);
        window.setTimeout(MoveCustomizedPackingSpecsBelowPackSpecName, 3000);
        window.setInterval(FixCustomizedProductAttributesWidth, 1000);
        window.setInterval(MoveCustomizedPackingSpecsBelowPackSpecName, 1000);
        if (window.Sys && Sys.WebForms && Sys.WebForms.PageRequestManager) {
            ...
            Sys.WebForms.PageRequestManager.getInstance().add_endRequest(MoveCustomizedPackingSpecsBelowPackSpecName);
        }
    </script>
    <style type="text/css">
        ...
        #GRP_CUS_ADRS_ADRS .divcol-M label {
            width: 15% !important;
            min-width: 130px;
        }

        #GRP_CUS_ADRS_ADRS .divcol-M textarea {
            width: min(81%, calc(100% - 145px)) !important;
        }
        ...
        [id$="pnlControls"] #GRP_CUS_ADRS_OTH .div2col-M select:not(#CAD_COUNTRY) {
            width: min(58%, calc(100% - 165px)) !important;
            min-width: 0 !important;
            max-width: none !important;
        }
        ...
        [id$="pnlControls"] #GRP_CUS_ADRS_OTH #CAD_COUNTRY {
            width: min(58.8%, calc(100% - 158px)) !important;
            min-width: 0 !important;
            max-width: 494px !important;
        }

        [id$="pnlControls"] #GRP_ADRS_ENTRY #CAD_TYPE,
        [id$="pnlControls"] #GRP_CUS_ADRS_OTH #CAD_COUNTRY {
            width: min(58.8%, calc(100% - 158px)) !important;
            min-width: 0 !important;
            max-width: 494px !important;
        ...
        [id$="pnlControls"] #GRP_CUS_ADRS_OTH #CAD_TAX_NO,
        [id$="pnlControls"] #GRP_CUS_ADRS_OTH #CAD_TAX_ID {
            width: min(52%, calc(100% - 220px)) !important;
            min-width: 0 !important;
        ...
        [id$="pnlControls"] #GRP_ADRS_ENTRY #CAD_NAME {
            width: min(79.4%, calc(100% - 158px)) !important;
            min-width: 0 !important;
            box-sizing: border-box;
        }
        ...
        #GRP_ADRS_LIST .fields-group > table:first-child td {
            display: block;
            width: 100% !important;
        }

        #GRP_ADRS_LIST .fields-group > table:first-child td:empty {
            display: none;
        }

        #GRP_ADRS_LIST .fields-group > table:first-child .div2col-M input[type="text"],
        #GRP_ADRS_ENTRY #CAD_NAME {
            width: min(79.4%, calc(100% - 158px)) !important;
            min-width: 0 !important;
            box-sizing: border-box;
        }

        #GRP_ADRS_LIST .fields-group > table:first-child .div2col-M label,
        #GRP_ADRS_ENTRY label[for="CAD_NAME"],
        #GRP_ADRS_ENTRY #lblCAD_NAME {
            width: 15% !important;
            min-width: 130px;
        }
        ...
        [id$="pnlControls"] #GRP_TC_NAME #TCH_DESC {
            width: min(79%, calc(100% - 170px)) !important;
            min-width: 0 !important;
            box-sizing: border-box;
        }
        ...
        [id$="pnlControls"] #GRP_TC_NAME #TCH_TYPE {
            width: min(57.5%, calc(100% - 210px)) !important;
            min-width: 0 !important;
            box-sizing: border-box;
        }
        ...
        [id$="pnlControls"] #GRP_PACK_SPEC_OTH .fields-group > table,
        [id$="pnlControls"] [id$="GRP_PACK_SPEC_OTH"] .fields-group > table,
        [id$="pnlControls"] #GRP_PACK_SPEC_OTH .fields-group > table tr,
        [id$="pnlControls"] [id$="GRP_PACK_SPEC_OTH"] .fields-group > table tr,
        [id$="pnlControls"] #GRP_PACK_SPEC_OTH .fields-group > table td,
        [id$="pnlControls"] [id$="GRP_PACK_SPEC_OTH"] .fields-group > table td {
            display: block;
            width: 100% !important;
            box-sizing: border-box;
        }
        ...
        [id$="pnlControls"] #GRP_PACK_SPEC_OTH .div2col-M,
        [id$="pnlControls"] #GRP_PACK_SPEC_OTH .divcol-M0,
        [id$="pnlControls"] [id$="GRP_PACK_SPEC_OTH"] .div2col-M,
        [id$="pnlControls"] [id$="GRP_PACK_SPEC_OTH"] .divcol-M0 {
            display: grid !important;
            grid-template-columns: 240px minmax(0, 1fr);
            column-gap: 10px;
            align-items: center;
        ...
        [id$="pnlControls"] #GRP_PACK_SPEC_OTH .div2col-M label,
        [id$="pnlControls"] #GRP_PACK_SPEC_OTH .divcol-M0 label,
        [id$="pnlControls"] [id$="GRP_PACK_SPEC_OTH"] .div2col-M label,
        [id$="pnlControls"] [id$="GRP_PACK_SPEC_OTH"] .divcol-M0 label {
            display: block;
            width: 240px !important;
            min-width: 240px !important;
            max-width: 240px !important;
        ...
        [id$="pnlControls"] #GRP_BRAND_DESC #CIM_DESC {
            width: min(75%, calc(100% - 260px)) !important;
            min-width: 0 !important;
            max-width: none !important;
            box-sizing: border-box;
        }
        ...
        [id$="pnlControls"] #GRP_IGPL_PRODUCT #CIM_ITEM {
            width: min(74%, calc(100% - 260px)) !important;
            min-width: 0 !important;
            box-sizing: border-box;
        }
        ...
        [id$="pnlControls"] #GRP_BRAND_ENTRY #CIM_AQL,
        [id$="pnlControls"] #GRP_BRAND_DESC #CIM_BRAND_SIZE,
        [id$="pnlControls"] #GRP_BRAND_DESC #CIM_BRAND_GROUP,
        [id$="pnlControls"] #GRP_PRD_ATRBT #CIM_THICKNESS,
        [id$="pnlControls"] #GRP_PRD_ATRBT #CIM_SURFACE,
        [id$="pnlControls"] #GRP_PRD_ATRBT #CIM_SIZE {
            width: min(50%, calc(100% - 260px)) !important;
            min-width: 0 !important;
            box-sizing: border-box;
        }
        ...
        [id$="pnlControls"] #GRP_PACK_SPEC #CIM_QC_SPEC_NAME,
        [id$="pnlControls"] #GRP_ADDNL_PACK_SPEC #CIM_STRAPPING_COLOUR {
            width: min(50%, calc(100% - 260px)) !important;
            min-width: 0 !important;
            box-sizing: border-box;
        }
            ...
            <div class="content-wrapper">
                ...
                <asp:Table runat="server" ID="tblTemplate" CssClass="asptbllinks">
                    <asp:TableRow ID="PageAction_Entry" runat="server">
                        <asp:TableCell>
                            <asp:Panel ID="pnlControls" runat="server" EnableViewState="false">
                ...
                <div id="diverror" style="display: none">
                    ...
                    <div id="divValidationSummary" runat="server" EnableViewState="false">
