...
namespace ERPSMS_v01.OrderPlanning
{
    public partial class OrderPlanning : ERP.Store.UI.WorkFlowBasePage
    {
            ...
            try
            {
                ...
                switch (commonActions)
                {
                    ...
                    case ActionsEnum.REVISE:
                        if (!IsValid)
                        {
                            ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowError", "ClosePopup();ShowErrorMessage('" + ERP.QC.Utilities.CommonFunctions.FormatErrorMessage(litErrorMsg.Text) + "','" + Resources.Captions.Information + "');", true);
                        }
                        else
                        {
                    ...
                    case ActionsEnum.EDIT_ACTION:
                        if (((ImageButton)sender).ID == "imbLineEdit")
                        {
                            ...
                            txtLineFromDt.Text = Convert.ToDateTime(objLine.PNL_FROM_DATE).ToString(CommonConstants.DATE_FORMAT_DD_MMM_YYYY);
                            txtLineFromTime.Text = Convert.ToDateTime(objLine.PNL_FROM_DATE).ToString(CommonConstants.TIMEFORMAT);
                            txtLineToDt.Text = Convert.ToDateTime(objLine.PNL_TO_DATE).ToString(CommonConstants.DATE_FORMAT_DD_MMM_YYYY);
            ...
            try
            {
                ...
                switch (ControlType)
                {
                    ...
                    case ControlsEnum.MACHINEFILL:
                        dtPage = ERP.Production.BL.BinCardBL.BinRegisterListBL.GetAllLine(currentUser.CurrentSBUPK, DeptPk: currentUser.CurrentDeptPK);
                        break;
                    case ControlsEnum.SCENARIODETAILS:
                        ...
                        string XmlReturn = OrderPlanningBL.GetScnarioDetails(xmlDoc.InnerXml, ref ReturnVal);
                    ...
                    case ControlsEnum.GROUPLINES:
                        ...
                        objLineBO = OrderPlanningBL.GetGroupLines(LinexmlDoc.InnerXml);
                        break;
                    case ControlsEnum.PLANGROUPLINES:
                        ...
                        objLineBO = OrderPlanningBL.GetGroupLines(PlanLinexmlDoc.InnerXml);
                        break;
                    case ControlsEnum.PLANCALCULATION:
                        ...
                        string XmlReturnPlan = OrderPlanningBL.GetScnarioDetails(xmlDocPlan.InnerXml, ref Return);
            ...
            switch (ControlType)
            {
                case ControlsEnum.EDIT:
                    if (objPlanDtls != null)
                    {
                        ...
                        txtFromDate.Text = Convert.ToDateTime(objPlanDtls.PNH_FROM_DT).ToShortDateString();
                        txtToDate.Text = Convert.ToDateTime(objPlanDtls.PNH_TO_DT).ToShortDateString();
        ...
        private void BindDropDown(ControlsEnum controlType)
        {
            switch (controlType)
            {
                case ControlsEnum.LINELIST:
                    ...
                    {
                        ddlLine.DataSource = ERP.QC.Utilities.CommonFunctions.HtmlDecodeDataTable(dtLine, ERP.Production.Utilities.Constants.DA.OrderPlanning.OrderPlanning.F_LNE_CODE);
                        ddlLine.DataTextField = ERP.Production.Utilities.Constants.DA.OrderPlanning.OrderPlanning.F_LNE_CODE;
                        ddlLine.DataValueField = ERP.Production.Utilities.Constants.DA.OrderPlanning.OrderPlanning.F_LNE_PK;
                        ddlLine.DataBind();
                    }
            ...
            foreach (GridViewRow gvr in grdGrouplist.Rows)
            {
                ...
                else
                {
                    if (!string.IsNullOrEmpty(txtTotalCurrPlan.Text) && Convert.ToDouble(txtTotalCurrPlan.Text) > 0)
                    {
                        ...
                        {
                            percVal = ((dtl.SOD_BAL_TO_PLAN * 100) / TotalBal).ToString();
        ...
        private void SavePlan(OrderPlanBO objOrderPlan, int NewModeSave)
        {
            ...
            if (objOrderPlan != null)
            {
                ...
                result = OrderPlanningBL.SaveOrderPlan(xmlDoc.InnerXml);
            }
            if (result > 0)
            {
                if (NewModeSave == 1)
