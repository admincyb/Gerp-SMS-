...
{
    public partial class StoreAdjustmentReport : System.Web.UI.Page
    {
        ...
        private void FillReport(int storeAdjID)
        {
            RptStoreAdjustment.Visible = true;
            LocalReport locRpt;
            string storeAdjDtls;

            storeAdjDtls = BusinessLogic.StoreManagement.StockAdjustment.GetStockAdjustmentDetailsForReport(storeAdjID);
            DataSet dsStoreAdjDetails = new DataSet();
            dsStoreAdjDetails.ReadXml(new XmlTextReader(new StringReader(storeAdjDtls)));
            if (dsStoreAdjDetails != null)
            {
                if (dsStoreAdjDetails.Tables.Count > 0)
                {
                    RptStoreAdjustment.Visible = true;
                    ReportDataSource dsStoreReqHeader = new ReportDataSource("StockAdjHeader", dsStoreAdjDetails.Tables[0]);
                    ReportDataSource dsStoreReqDetail = new ReportDataSource("StockAdjDetail", dsStoreAdjDetails.Tables[1]);
                    locRpt = null;
                    locRpt = RptStoreAdjustment.LocalReport;
                    locRpt.ReportPath = string.Empty;
                    locRpt.ReportPath = Server.MapPath("../Reports/StoreAdjustmentRpt.rdlc");
                    RptStoreAdjustment.Height = 500;
                    RptStoreAdjustment.LocalReport.DataSources.Clear();
                    ...
                    locRpt.SetParameters(parameters);
                    if (dsStoreAdjDetails != null & dsStoreAdjDetails.Tables[0].Rows.Count > 0)
                    {
                        int CompanyPK = Convert.ToInt32(dsStoreAdjDetails.Tables[0].Rows[0]["SAH_BIZUNIT"].ToString());
                        RptStoreAdjustment.LocalReport.DataSources.Add(GetCompanyDetails(CompanyPK));
                    }
                    RptStoreAdjustment.LocalReport.DataSources.Add(dsStoreReqHeader);
                    RptStoreAdjustment.LocalReport.DataSources.Add(dsStoreReqDetail);
                    RptStoreAdjustment.ViewStateMode = System.Web.UI.ViewStateMode.Enabled;
                    RptStoreAdjustment.LocalReport.Refresh();

                    SavePDF(locRpt, "StoreAdjustment" + storeAdjID);
                ...
                {
                    RptStoreAdjustment.Visible = false;
