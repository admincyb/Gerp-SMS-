...
{
    public partial class StoreAuditingReport : ERP.Store.UI.MyBasePage
    {
        ...
        {
            RptStoreAuditing.Visible = true;
            LocalReport locRpt;
            DataSet dsStoreAudit = new DataSet();
            dsStoreAudit = BusinessLogic.StoreManagement.StoreAudit.GetStoreAuditReportDetails(storeAdjID);
            if (dsStoreAudit != null)
            {
                if (dsStoreAudit.Tables.Count > 0)
                {
                    RptStoreAuditing.Visible = true;
                    ReportDataSource dsStoreReqHeader = new ReportDataSource("StoreAuditHeader", dsStoreAudit.Tables[0]);
                    ReportDataSource dsStoreReqDetail = new ReportDataSource("StoreAuditDetail", dsStoreAudit.Tables[1]);
                    locRpt = null;
                    locRpt = RptStoreAuditing.LocalReport;
                    locRpt.ReportPath = string.Empty;
                    locRpt.ReportPath = Server.MapPath("../Reports/StoreAuditReport.rdlc");
                    RptStoreAuditing.Height = 500;
                    RptStoreAuditing.LocalReport.DataSources.Clear();
                    ...
                    if (dsStoreAudit != null & dsStoreAudit.Tables[0].Rows.Count > 0)
                    {
                        int CompanyPK = Convert.ToInt32(dsStoreAudit.Tables[0].Rows[0]["SAH_BIZUNIT"].ToString());
                        RptStoreAuditing.LocalReport.DataSources.Add(GetCompanyDetails(CompanyPK));
                    }
                    RptStoreAuditing.LocalReport.DataSources.Add(dsStoreReqHeader);
                    RptStoreAuditing.LocalReport.DataSources.Add(dsStoreReqDetail);
                    RptStoreAuditing.ViewStateMode = System.Web.UI.ViewStateMode.Enabled;
                    RptStoreAuditing.LocalReport.Refresh();

                    SavePDF(locRpt, "StoreAuditing" + storeAdjID);
                ...
                {
                    RptStoreAuditing.Visible = false;
