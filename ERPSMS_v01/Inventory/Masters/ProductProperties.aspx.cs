    ...
    public partial class ProductProperties : System.Web.UI.Page
    {
        ...
        private void PageActionHandler()
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    {
                        grpType = Int32.TryParse(Request.QueryString[QueryStrings.Type], out grpType) == true ? grpType : 1;
                    }
                    else
                    {
                        grpType=1;
                    }
                    hdnGrptype.Value = grpType.ToString();
        ...
        private ADM_CONST_MST SetUIValuesToObject()
        {
            try
            {
                ...
                admConstMstObj.CON_ACTIVE = (byte)DbActiveStatus.ACTIVE;
        ...
        public void SearchGrid()
        {
            try
            {
                if (admConstMstList != null)
                {
                    ...
                    {
                        admConstMstList = (from c in admConstMstList where c.CON_CODE.ToLower().Contains(txtSearchBy.Text.Trim().ToLower()) || c.CON_NAME.ToLower().Contains(txtSearchBy.Text.Trim().ToLower()) select c).ToList();
                    }
                    grdProductionMst.DataSource = admConstMstList;
                    grdProductionMst.DataBind();
                    uclPaging.Visible = true;
        ...
        public void BindGroupDropDown()
        {
            ...
            {
                ddlGroup.Items.Add(new ListItem(Resources.Messages.Msg_EmptyGrid, "0"));
            }
        }
