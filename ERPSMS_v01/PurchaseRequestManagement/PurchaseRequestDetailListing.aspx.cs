...
{
    public partial class PurchaseRequestDetailListing : ERP.Store.UI.MyBasePage
    {
            ...
            try
            {
                if (!IsPostBack)
                {
                    ...
                    if (Request.QueryString["Sbu"] != null)
                        hdfSbu.Value = Request.QueryString["Sbu"].ToString();
            ...
            try
            {
                switch (type)
                {
                    ...
                    case ControlsEnum.DEFAULT:
                        ...
                        gridParam.PRH_TYPE = Request.QueryString["Type"] == null ? 0 : Convert.ToInt32(Request.QueryString["Type"]);

                        processId = GetNullableInt(hdfProcId.Value) ?? 0;
                        pageUrl = Resources.PageURL.PurchaseRequest; // "/PurchaseRequestManagement/PurchaseRequestCreation.aspx"; 
                        if (Request.QueryString["Type"] != null)
                        {
                            pageUrl += "?Type=" + Request.QueryString["Type"].ToString();
                            if (Request.QueryString["Sbu"] != null)
                        ...
                        dsPageData = DataAccess.PurchaseRequestManagement.PurchaseRequestListingDL.GetPurchaseRequestList(gridParam, currentUser, pageUrl, processId, transactionStatus, reqStore, ioNo
                            , itemName, reqDept, reqBy, filterStatus, prNo, cmpPk, POCategory, prGroup);

                        break;
                    #endregion
                    #region Transaction Status
                    case ControlsEnum.TRANSACTIONSTATUS:
                        dtPageData = CommonBL.GetAppStatus("PR", string.Empty);
                        break;
                    #endregion
                    #region Plant
        ...
        protected void ActionHandler(object sender, EventArgs e)
        {
            ...
            switch (commonActions)
            {
                ...
                case ActionsEnum.NEW:
                    string RequestURL = GetLocalResourceObject("PurchaseRequestEntryURL").ToString();
                    if (Request.QueryString["Type"] != null)
                    {
            ...
            try
            {
                ...
                if ((sender as GridView).ID == "grdPRList")
                {
                    if (e.Row.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        int isChangePR = Convert.ToInt32(hdfChangePR.Value);
                        ...
                        if (isChangePR == 1 && UserStatus == 0 && prhLinkStatus == 0 && PRH_STATUS != 4 && PRH_STATUS != 5)
                        {
                ...
                switch (e.CommandName)
                {
                    case "PERFORMACTION":
                        ...
                        if (UserStatus == 1)
                        {
                            ...
                            RequestURL = GetLocalResourceObject("PurchaseRequestEntryURL").ToString() + "?RefID=" + refID;
                        }
                        else if (UserStatus == 2)
                        {
                            //Response.Redirect(GetLocalResourceObject("PurchaseRequestEntryURL").ToString() + "?PK=" + pk + "&Dep=" + storePK);
                            RequestURL = GetLocalResourceObject("PurchaseRequestEntryURL").ToString() + "?PK=" + pk + "&Dep=" + storePK;
                        }
                        if (Request.QueryString["Type"] != null)
                        {
                            RequestURL += "&Type=" + Request.QueryString["Type"].ToString();
                    ...
                    case "SHORTCLOSEPR":
                        #region SHORTCLOSEPR
                        pageUrl = GetLocalResourceObject("PurchaseRequestEntryURL").ToString();
                        if (Request.QueryString["Type"] != null)
                        {
                            pageUrl += "?Type=" + Request.QueryString["Type"].ToString();
                            if (Request.QueryString["Sbu"] != null)
                            ...
                            pageUrl += "&RefID=" + refID + "&Status=1&IsModify=1";
                        }
                        else
                            pageUrl += "?RefID=" + refID + "&Status=1&IsModify=1";
                    ...
                    case "VIEW":
                        ...
                        if (UserStatus == 1)
                        {
                            ...
                            RequestURL = GetLocalResourceObject("PurchaseRequestEntryURL").ToString() + "?RefID=" + refID + "&Status=1";
                        }
                        else if (UserStatus == 2)
                        {
                            //Response.Redirect(GetLocalResourceObject("PurchaseRequestEntryURL").ToString() + "?PK=" + pk + "&Status=1");
                            RequestURL = GetLocalResourceObject("PurchaseRequestEntryURL").ToString() + "?PK=" + pk + "&Status=1";
                        }
                        else if (UserStatus == 0)
                        {
                            if (refID == 0)
                    ...
                    case "CHANGE":
                        #region CHANGE
                        int store = Convert.ToInt32((row.FindControl("hdfPrhDept") as HiddenField).Value);
                        RequestURL = GetLocalResourceObject("PurchaseRequestEntryURL").ToString() + "?RefID=" + refID + "&Modify=1&Status=" + UserStatus;
        ...
        private int FillProcessId()
        {
            ...
            if (Request.QueryString["Type"] != null)
            {
                ...
                if (Request.QueryString["Sbu"] != null)
                {
                    path += "&Sbu=" + Request.QueryString["Sbu"].ToString();
                }
            }
        ...
        private void GetUserRights()
        {
            ...
            if (usrRights.Rights.Count > 0)
            {
                foreach (var item in usrRights.Rights)
                {
                    ...
                    if (item.ActionName == "CHANGE" && item.HasActionRight == true)//For Cancelling PR
                    {
                        hdfChangePR.Value = "1";
                    }
                }
