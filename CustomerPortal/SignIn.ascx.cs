...
namespace LatexERPV2
{
    public partial class SignIn : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //TextBox UserName = (TextBox)this.LgnUser.FindControl("UserName");
            //UserName.Focus();
            gERPUserID.Focus();
        ...
        private void Authenticate(User uLogin)
        {
            ...
            //{
            //    //redirectURL = "~/Dashboard/Dashboard.aspx";
            //    redirectURL = Resources.PageURL.CustomerPortalAfterLogin;
            //}
            //else if (System.Configuration.ConfigurationManager.AppSettings["ERPURL"] != null && Request.QueryString["IsLogin"] == null)
            //{
            //    ERP.Utilities.CryptoServices crypto = new ERP.Utilities.CryptoServices();
            //    string password = HttpUtility.HtmlEncode(crypto.EncryptString(this.gERPPwd.Text.Trim(), System.Configuration.ConfigurationManager.AppSettings["salt"]));
            //    redirectURL = System.Configuration.ConfigurationManager.AppSettings["ERPURL"] + "?PostUser=" + HttpUtility.UrlEncode(CommonFunctions.Encrypt(gERPUserID.Text)) + "&PostPassword=" + HttpUtility.UrlEncode(password)
            //        + "&IsLogin=true";
            //}
            //else
            //    redirectURL = "~/AccountManagement/Inbox.aspx";
            ...
            isLoginCustomerPortal = true;

            if (Request.QueryString["isLoginSMS"] != null && !string.IsNullOrEmpty(Request.QueryString["isLoginSMS"]))
            {
            ...
            #endregion
            #region password Expiry checking
        ...
        protected void LgnUser_Authenticate(object sender, System.Web.UI.WebControls.AuthenticateEventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    //SignIn.logger.Log(NLog.LogLevel.Fatal, "Error Occured " + ErrMsg);
                    // ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowError", "ShowInvalidMessage('" + ErrorText.Text + "');", true);
                }
            }
        ...
        private void DisablePresentaionDiv()
        {
            //  divPresentation.Visible = false;

            try
            {
                string enable = GetGlobalResourceObject("ConfigurationsRes", "EnablePresentation").ToString();
                divPresentation.Visible = (enable == 1.ToString() ? true : false);
            }
            catch (Exception ex)
            { }


            //DataTable dtConfig = BusinessLogic.CommonManagement.CommonBL.GetCurrencyConfiguration("LOGIN SETTINGS", "SHOW LOGIN PRESENTATION", 0);
            //if (dtConfig != null && dtConfig.Rows.Count > 0)
            //{
            //    string enable = dtConfig.Rows[0]["ACF_VALUE"].ToString();
            //    divPresentation.Visible = (enable == 1.ToString() ? true : false);
            //}
        }
    }
