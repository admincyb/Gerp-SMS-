...
{
    public partial class SaleOrderDetails : ERP.Store.UI.WorkFlowBasePage
    {
        ...
        SaleOrderDetailsBO contractDetails;
        int selectedDtlPK;
        int selectedCusItemPK;
        int selectedSCItemPK;
                    ...
                    case ControlsEnum.SALEORDERHEADER:
                        if (SaleOrderHeaderSession != null)
                        {
                            ...
                            if (ddlAgent.SelectedValue != CommonConstants.SELECTVAL)
                            {
                                ...
                                saleOrderHeaderObj.SOH_SHIPPING_NAME = hdfAgentName.Value;
                                saleOrderHeaderObj.SOH_SHIPPING_ADDRESS = hdfAgentAddress.Value;
                                if (hdfAgentCountry.Value.Trim() != string.Empty)
                                    saleOrderHeaderObj.SOH_SHIPPING_COUNTRY = hdfAgentCountry.Value.Trim();
                                saleOrderHeaderObj.SOH_SHIPPING_COUNTRY_TEXT = hdfAgentCountryText.Value;
                                saleOrderHeaderObj.SOH_SHIPPING_EMAIL = hdfAgentEmail.Value;
                                saleOrderHeaderObj.SOH_SHIPPING_FAX = hdfAgentFax.Value;
                                saleOrderHeaderObj.SOH_SHIPPING_MOBILE = hdfAgentMobile.Value;
                                saleOrderHeaderObj.SOH_SHIPPING_PHONE = hdfAgentPhone.Value;
                                saleOrderHeaderObj.SOH_SHIPPING_ZIP = hdfAgentZip.Value;
                            }
                        ...
                        if (SaleOrderHeaderSession.SaleOrderDetails != null && SaleOrderHeaderSession.SaleOrderDetails.Count > 0)
                        {
                            foreach (GridViewRow grdrow in grdItemDetails.Rows)
                            {
                                ...
                                if (saleOrderDetailsObj != null)
                                {
                                    ...
                                    if (txtLotNo != null)
                                        saleOrderDetailsObj.SOD_LOT_NO = System.Web.HttpUtility.HtmlEncode(txtLotNo.Text);
                                    ...
                                    if (hdfArtWork != null && hdfArtWork.Value != string.Empty && hdfArtWork.Value != CommonConstants.SELECT_VALUE_ZERO)
                ...
                switch (controlType)
                {
                    case ControlsEnum.SALEORDERHEADER:
                        if (saleOrderHeaderObj != null)
                        {
                            ...
                            txtHdrDiscount.Text = saleOrderHeaderObj.SOH_TOTAL_DISCOUNT.ToString("c");
                            txtHdrTax.Text = saleOrderHeaderObj.SOH_TOTAL_TAX.ToString("c");
                            txtShipping.Text = saleOrderHeaderObj.SOH_TOTAL_SHIP_CHARGE.ToString("c");
                            txtPriceAdj.Text = saleOrderHeaderObj.SOH_TOTAL_ADJUST.ToString("c");
                            txtHdrTotal.Text = saleOrderHeaderObj.SOH_NET_AMOUNT.ToString("c");
                            ...
                            if (!string.IsNullOrEmpty(saleOrderHeaderObj.SOH_SHIPPING_NAME))
                                hdfAgentName.Value = saleOrderHeaderObj.SOH_SHIPPING_NAME;
                            if (!string.IsNullOrEmpty(saleOrderHeaderObj.SOH_SHIPPING_COUNTRY))
                                hdfAgentCountry.Value = saleOrderHeaderObj.SOH_SHIPPING_COUNTRY;
                            hdfAgentAddress.Value = saleOrderHeaderObj.SOH_SHIPPING_ADDRESS;

                            if (!string.IsNullOrEmpty(saleOrderHeaderObj.SOH_SHIPPING_COUNTRY_TEXT))
                                hdfAgentCountryText.Value = saleOrderHeaderObj.SOH_SHIPPING_COUNTRY_TEXT;
                            if (!string.IsNullOrEmpty(saleOrderHeaderObj.SOH_SHIPPING_EMAIL))
                                hdfAgentEmail.Value = saleOrderHeaderObj.SOH_SHIPPING_EMAIL;
                            if (!string.IsNullOrEmpty(saleOrderHeaderObj.SOH_SHIPPING_FAX))
                                hdfAgentFax.Value = saleOrderHeaderObj.SOH_SHIPPING_FAX;
                            if (!string.IsNullOrEmpty(saleOrderHeaderObj.SOH_SHIPPING_MOBILE))
                                hdfAgentMobile.Value = saleOrderHeaderObj.SOH_SHIPPING_MOBILE;
                            if (!string.IsNullOrEmpty(saleOrderHeaderObj.SOH_SHIPPING_PHONE))
                                hdfAgentPhone.Value = saleOrderHeaderObj.SOH_SHIPPING_PHONE;
                            if (!string.IsNullOrEmpty(saleOrderHeaderObj.SOH_SHIPPING_ZIP))
                                hdfAgentZip.Value = saleOrderHeaderObj.SOH_SHIPPING_ZIP;
            ...
            try
            {
                switch (controlType)
                {
                    ...
                    case ControlsEnum.TAXPOPUPGRID:
                        if (IsHeaderTax)
                        {
                            contractTaxHdrList = SaleOrderHeaderSession.TaxHdr.Where(tax => Convert.ToInt32(tax.SLT_TAX_CATEGORY) == Convert.ToInt32(hdfTaxCategory.Value)).ToList();
                        }
                        else
                        {
                            contractDetails = SaleOrderHeaderSession.SaleOrderDetails.SingleOrDefault(dtl => dtl.SOD_PK == selectedDtlPK
                                && dtl.SOD_CUST_ITEM == selectedCusItemPK && dtl.SOD_ITEM == selectedSCItemPK);
                            if (contractDetails != null)
                            {
                                contractTaxHdrList = contractDetails.TaxDtl.Where(tax => Convert.ToInt32(tax.SLT_TAX_CATEGORY) == Convert.ToInt32(hdfTaxCategory.Value)).ToList();
                            }
                        }
            ...
            try
            {
                ...
                switch (commonActions)
                {
                    ...
                    case ActionsEnum.TAXDETAILS:
                        ...
                        hdfSODPK = (((sender as ImageButton).Parent.Parent as GridViewRow).FindControl("hdfSODPK") as HiddenField);
                        ...
                        selectedDtlPK = string.IsNullOrEmpty(hdfSODPK.Value) ? 0 : Convert.ToInt32(hdfSODPK.Value);
                        selectedCusItemPK = string.IsNullOrEmpty(hdfCusItemPK.Value) ? 0 : Convert.ToInt32(hdfCusItemPK.Value);
                        selectedSCItemPK = string.IsNullOrEmpty(hdfItemPK.Value) ? 0 : Convert.ToInt32(hdfItemPK.Value);
                        if (SaleOrderHeaderSession != null)
                        {
                            IsHeaderTax = false;
