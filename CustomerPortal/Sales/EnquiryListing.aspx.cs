...
{
    public partial class EnquiryListing : ERP.Store.UI.WorkFlowBasePage
    { 
        ...
        /// <summary>
        /// To maintain the query string
        /// </summary>
        public string STATUS
        {
            get
            {
                return this.Request.QueryString["STATUS"] == null ? "" : Request.QueryString["STATUS"];
            }
            set
            {
                this.Request.QueryString["STATUS"] = value;
            }
        }
        #endregion
        ...
        protected void ActionHandler(object sender, EventArgs e)
        {
            ...
            switch (commonActions)
            {
                ...
                case ActionsEnum.ITEMSELECTED:
                    ...
                    selectedPK = Convert.ToInt32(grdEnquiryList.DataKeys[selectedGrdrow.RowIndex].Values[0]);
            ...
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow || e.Row.RowType == DataControlRowType.Header)
                {
                    if (EnqStatus == 1)
                    {
                        ...
                        e.Row.Cells[5].Width = Unit.Percentage(32);
                        e.Row.Cells[6].Width = Unit.Percentage(19);
                        e.Row.Cells[7].Width = Unit.Percentage(17);
                        if (e.Row.RowType == DataControlRowType.DataRow)
                        {
                            lblCustomerName = e.Row.Cells[5].FindControl("lblCustomerName") as Label;
                            lblCustomerName.Text = ERP.Utilities.CommonFunctions.GetShortString(lblCustomerName.Text, 53);

                            lblCustCountry = e.Row.Cells[6].FindControl("lblCustCountry") as Label;
                            lblCustCountry.Text = ERP.Utilities.CommonFunctions.GetShortString(lblCustCountry.Text, 35);

                            lblRemarks = e.Row.Cells[7].FindControl("lblRemarks") as Label;
                            lblRemarks.Text = ERP.Utilities.CommonFunctions.GetShortString(lblRemarks.Text, 25);
                        }
                    }
                    else
                    {
                        if (e.Row.RowType == DataControlRowType.DataRow)
                        {
                            ...
                            lblCustCountry = e.Row.Cells[6].FindControl("lblCustCountry") as Label;
                            lblCustCountry.Text = ERP.Utilities.CommonFunctions.GetShortString(lblCustCountry.Text, 18);
        ...
        private void FillProcessId()
        {
            string path;
            if (EnqStatus == 2)
                path = GetLocalResourceObject("WkfQuotationUrl").ToString();
            else
                path = GetLocalResourceObject("WkfEnquiryUrl").ToString();
        ...
        private void SetUIEditView(ActionsEnum mode)
        {
            try
            {
                foreach (GridViewRow grdrow in grdEnquiryList.Rows)
                {
                    ...
                    if (rbtn.Checked)
                    {
                        selectedPK = Convert.ToInt32(grdEnquiryList.DataKeys[grdrow.RowIndex].Values[0]);
                        Session[ERP.Utilities.SessionStrings.ENQUIRYPK] = selectedPK;
                        switch (mode)
                        {
                            case ActionsEnum.QUOTE:
                            case ActionsEnum.EDIT:
                            ...
                            case ActionsEnum.PRINT:
                                ...
                                ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "Openwindow", "OpenPDF('" + "../Reports/GenerateReport.aspx?ID=" + selectedPK.ToString() + "&APPTYPE=" + hdfAppType.Value + "&APPSUBTYPE=" + hdfAppSubType.Value + "');", true);
                                return;
                                break;
                        }
