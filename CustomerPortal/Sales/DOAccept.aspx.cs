...
{
    public partial class DOAccept : ERP.Store.UI.WorkFlowBasePage
    {
            ...
            try
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    if (CurrPK > 0)
                    {
                        ...
                        if (lblQuotationNo.Text.Trim().Equals(string.Empty))
                        {
            ...
            try
            {
                switch (type)
                {
                    ...
                    case ControlsEnum.QUOTATION:
                        ...
                        {
                            ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowError", "AlertMessage('" + Resources.ErrorMessages.Msg_Edit_Delete + "','" + Resources.Captions.Information + "','DOAccept.aspx');", true);
                        }
                        break;
                    case ControlsEnum.QUOTATIONTAXTYPES:
                        ...
                        else
                        {
                            if ((int)TaxType.Tax == category)
                            {
                                ...
                                //dtQuotationTaxDetails = BusinessLogic.Administration.Masters.TaxSettingsMaster.GetActiveTaxCategoryDateValue(category, currentUser.SBUID, Convert.ToByte(DbActiveStatus.ACTIVE), Convert.ToDateTime(txtQuotationDate.Text), 0, TaxFilterType.SAL);
                            }
                            else
                            {
                                dtQuotationTaxDetails = BusinessLogic.Administration.Masters.TaxSettingsMaster.GetActiveTaxCategoryDateValue(category, currentUser.SBUID, Convert.ToByte(DbActiveStatus.ACTIVE), Convert.ToDateTime(txtQuotationDate.Text), 0, string.Empty, 1, 0, 1);
            ...
            try
            {
                switch (controlType)
                {
                    case ControlsEnum.QUOTATIONHEADER:
                        ...
                        quotationHeaderObj.CEH_NO = string.IsNullOrEmpty(lblQuotationNo.Text.Trim()) ? string.Empty : lblQuotationNo.Text.Trim();
                        ...
                        quotationHeaderObj.CEH_TRX_STATUS = (byte)DirectOrderStatus.SalesOrder;
                ...
                switch (controlType)
                {
                    ...
                    case ControlsEnum.QUOTATIONHEADER:
                        if (quotationHeaderObj != null)
                        {
                            ...
                            lblQuotationNo.Text = quotationHeaderObj.CEH_NO.ToString();
                            lblEnquiryNo.Text = string.IsNullOrEmpty(quotationHeaderObj.CEH_REF_NO) ? string.Empty : HttpUtility.HtmlDecode(quotationHeaderObj.CEH_REF_NO.ToString());
                            hdfEnquiryNo.Value = string.IsNullOrEmpty(quotationHeaderObj.CEH_REF_NO) ? string.Empty : HttpUtility.HtmlDecode(quotationHeaderObj.CEH_REF_NO.ToString());
                            lblEnquiryDate.Text = string.IsNullOrEmpty(quotationHeaderObj.CEH_REF_DATE) ? string.Empty : quotationHeaderObj.CEH_REF_DATE.ToString();
                            hdfEnquiryDate.Value = string.IsNullOrEmpty(quotationHeaderObj.CEH_REF_DATE) ? string.Empty : quotationHeaderObj.CEH_REF_DATE.ToString();
                            hdfQuotationPK.Value = quotationHeaderObj.CEH_PK.ToString();
                            txtQuotationDate.Text = quotationHeaderObj.CEH_DATE;
                            lblBookingDate.Text = quotationHeaderObj.CEH_BOOKING_DATE;
                            ...
                            txtHdrDiscount.Text = quotationHeaderObj.CEH_TOTAL_DISCOUNT.ToString(hdfCurrencyFormat.Value);
                            txtHdrTax.Text = quotationHeaderObj.CEH_TOTAL_TAX.ToString(hdfCurrencyFormat.Value);
                            ...
                            txtHdrTotal.Text = quotationHeaderObj.CEH_NET_AMOUNT.ToString(hdfCurrencyFormat.Value);
                        ...
                        if (quotationDetailsObj != null)
                        {
                            ...
                            foreach (QuotationTaxHdr quotationTaxHdrObj in discDetail)
                            {
                                ...
                                if (!string.IsNullOrEmpty(taxFormula))
                                {
                                    ...
                                    quotationTaxHdrObj.ETD_TAX_AMT = Math.Round(StringToFormula(taxFormula), Thread.CurrentThread.CurrentCulture.NumberFormat.CurrencyDecimalDigits);
                                }
                            }
        ...
        private void SetSubTotal()
        {
            ...
            if (txtSubTotalFooter != null)
            {
                ...
                txtSubTotalFooter.Text = QuotationHeaderSession.CEH_TOTAL_AMT.ToString(hdfCurrencyFormat.Value);
            }
        }
                            ...
                            if (hasValidRate)
                            {
                                if (quotationHeaderObj != null && quotationHeaderObj.QuotationDtl != null)
                                {
                                    ...
                                    if (result > 0) // Success !  redirect to listing page
                                    {
                                        ...
                                        litErrorMsg.Text = string.Format(litErrorMsg.Text, Resources.PageNameRes.DirectOrderAccept);
                                        if (string.IsNullOrEmpty(lblQuotationNo.Text))
                                        {
                                        ...
                                        {
                                            ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "ShowErrorMsg", "ShowErrorMessage('" + litErrorMsg.Text
                                                + "','" + Resources.ErpRes.Information + "','" + Page.ResolveClientUrl(Resources.PageURL.DOAcceptListing) + "');", true);
                                        }
                                    }
                                    else
                                    {
            ...
            try
            {
                if ((sender as GridView).ID == "grdQuotation")
                {
                    if (e.Row.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        {
                            e.Row.Cells[10].Visible = EnableItemDiscount;
                            e.Row.Cells[11].Visible = EnableItemTax;
                            e.Row.Cells[9].Visible = EnableItemDiscount || EnableItemTax;

                            //lblItemCartonsOrBags = e.Row.FindControl("lblItemCartonsOrBags") as Label;

                            //qtnItem = (QuotationDetails)e.Row.DataItem;
                            //if (qtnItem != null)
                            //{
                            //    if (lblItemCartonsOrBags != null)
                            //    {
                            //        lblItemCartonsOrBags.Text = Math.Ceiling(qtnItem.CED_ENQ_QTY / qtnItem.APS_TOTAL_PCS).ToString();
                            //        lblItemCartonsOrBags.ToolTip = HttpUtility.HtmlDecode(qtnItem.PACKING_TEXT);
                            //    }
                            //}
                        }
                        if (QtnStatus == 11 || QtnStatus == 17)
                    ...
                    else if (e.Row.RowType == DataControlRowType.Footer && quotationDetailsList != null)
                    {
                        e.Row.Cells[10].Visible = EnableItemDiscount;
                        e.Row.Cells[11].Visible = EnableItemTax;
                        e.Row.Cells[9].Visible = EnableItemDiscount || EnableItemTax;
                        ...
                        //{
                        //    lblItemTotalQty.Text = lblItemTotalQty.ToolTip = quotationDetailsList.Sum(itm => itm.CED_ENQ_QTY).ToString("N");
                        //}
                        //if (lblItemTotalCarton != null)
                        //{
                        //    lblItemTotalCarton.Text = lblItemTotalCarton.ToolTip = quotationDetailsList.Sum(itm =>
                        //        Math.Ceiling(itm.CED_ENQ_QTY / itm.APS_TOTAL_PCS)).ToString();
                        //}
                        //if (lblItemTotalAmount != null)
                        //{
                        //    lblItemTotalAmount.Text = lblItemTotalAmount.ToolTip = quotationDetailsList.Sum(itm => itm.CED_AMOUNT).ToString("N");
                        //}
                        //if (lblDiscountTotal != null)
                        //{
                        //    lblDiscountTotal.Text = lblDiscountTotal.ToolTip = quotationDetailsList.Sum(itm => itm.CED_DISCOUNT).ToString("N");
                        //}
                        //if (lblTaxTotal != null)
                        //{
                        //    lblTaxTotal.Text = lblTaxTotal.ToolTip = quotationDetailsList.Sum(itm => itm.CED_TAX).ToString("N");
                        //}
                    }
