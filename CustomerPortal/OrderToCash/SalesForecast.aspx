<%@ Page Title="<%$ Resources:Captions,Title_SalesForecast %>" Language="C#" Theme="ClassicExt"
    EnableEventValidation="false" MasterPageFile="~/ERPSMS_2.Master" AutoEventWireup="true"
    CodeBehind="SalesForecast.aspx.cs" Inherits="CustomerPortal.OrderToCash.SalesForecast"
...
<asp:Content ID="cntMain" runat="server" ContentPlaceHolderID="MainContent">
    <asp:UpdatePanel ID="aupdpnlPacking" runat="server">
        <ContentTemplate>
                        ...
                        <asp:TableRow>
                            <asp:TableCell ID="SEC_ActionPanel" CssClass="SEC_ACTION" HorizontalAlign="Right">
                                ...
                                <ul id="pnlListing" runat="server">
                                    ...
                                    <li id="pnlPrint">
                                        <asp:Button runat="server" TabIndex="7" ID="btnPrint" CommandName="PRINT" OnClick="ActionHandler"
           ...
           <%--------------colpase btn----------%>
                   <table class="table-devide" id="tbladvancedSearch" style="margin-top: 8px;">
                       <tr id="Tr1" runat="server">
                          <td>
                            <div class="div2col-S">
                                <asp:Label ID="Label1" runat="server"  Text="<%$ resources:SelectMonth %>" AssociatedControlID="txtCalender"></asp:Label>
                                ...
                                <cc1:CalendarExtender ID="txtCalender_CalendarExtender" runat="server" BehaviorID="calendar1"
                                    OnClientDateSelectionChanged="checkDate" TargetControlID="txtCalender" Format="MMM-yyyy"
                                    OnClientShown="onCalendarShown" ClientIDMode="Static" OnClientHidden="onCalendarHidden">
                           ...
                           <div class="div2col-S">
                             <asp:Label ID="lblSStatus" runat="server" Text="<%$ resources:Status %>" AssociatedControlID="ddlStatus"></asp:Label>
                                <asp:DropDownList ID="ddlStatus" runat="server" TabIndex="2" CssClass="select-small">
                                  ...
                                  <asp:ImageButton ID="btnSearch" SkinID="search-ext" runat="server" Text="<%$ resources:Controls,Go %>"
          ...
          <%-- New Search Ends--%>
                      <div class="gridwrap">
                                <asp:GridView runat="server" ID="grdSalesForecastMst" Width="100%" PageSize="<%$ resources:PageSize %>"
                                    ...
                                    <Columns>
                                        <asp:TemplateField>
                                            ...
                                            <ItemTemplate>
                                                ...
                                                <asp:HiddenField ID="hfCusPK" runat="server" Value='<%#Eval(Resources.DataFieldRes.CustomerPK) %>' />
                                                <asp:HiddenField ID="hfCusEmail" runat="server" Value='<%#Eval(Resources.DataFieldRes.CustomerEmail) %>' />
                                            </ItemTemplate>
                                            <ItemStyle Width="1%" HorizontalAlign="Center" />
                                        </asp:TemplateField>
                                        <asp:TemplateField HeaderText="<%$ resources:CustomerName%>" SortExpression="<%$ resources:DataFieldRes,CustomerNameText %>">
                                            <ItemTemplate>
                                                <asp:Label ID="lblCustomerNameTxt" runat="server" ToolTip='<%# ERP.Utilities.CommonFunctions.GetEncodedString(Eval("CUS_EMAIL"))%>'
                                                    Text='<%# ERP.Utilities.CommonFunctions.GetShortString(ERP.Utilities.CommonFunctions.GetEncodedString(Eval(Resources.DataFieldRes.CustomerNameText)),40) %>' />
                                            </ItemTemplate>
                                            <ItemStyle Width="29%" />
                                        </asp:TemplateField>
                                        <asp:TemplateField HeaderText="<%$ resources:Country %>" SortExpression="<%$ resources:DataFieldRes,CustomerCountry %>">
                                            <ItemTemplate>
                                                <asp:Label ID="lblCustomerCountry" runat="server" ToolTip='<%# ERP.Utilities.CommonFunctions.GetEncodedString(Eval(Resources.DataFieldRes.CustomerCountry))%>'
                                                    Text='<%# ERP.Utilities.CommonFunctions.GetShortString(ERP.Utilities.CommonFunctions.GetEncodedString(Eval(Resources.DataFieldRes.CustomerCountry)),15) %>' />
                                            </ItemTemplate>
                                            <ItemStyle Width="11%" />
                                        </asp:TemplateField>
                                        <asp:TemplateField HeaderText="<%$ resources:OrderFreq %>" SortExpression="<%$ resources:DataFieldRes,CustomerOrdFreq %>">
                                            <ItemTemplate>
                                                <asp:Label ID="lblCustomerOrdFreq" runat="server" ToolTip='<%# Eval(Resources.DataFieldRes.CustomerOrdFreq) %>'
                                                    Text='<%#  ERP.Utilities.CommonFunctions.GetShortString(Eval(Resources.DataFieldRes.CustomerOrdFreq),11) %>' />
                                            </ItemTemplate>
                                            <ItemStyle Width="10%" />
                                        </asp:TemplateField>
                                        <asp:TemplateField HeaderText="<%$ resources:ShipDate %>" SortExpression="<%$ resources:DataFieldRes,ShipDate %>">
                                            <ItemTemplate>
                                                <asp:Label ID="lblShipDate" runat="server" ToolTip='<%# Eval(Resources.DataFieldRes.ShipDate, Resources.Constants.DateFormatGrid)  %>'
                                                    Text='<%#Eval(Resources.DataFieldRes.ShipDate, Resources.Constants.DateFormatGrid) %>' />
                                            </ItemTemplate>
                                            <ItemStyle Width="6%" />
                                        </asp:TemplateField>
                                        <asp:TemplateField HeaderText="<%$ resources:LastOrderReceivedon %>" SortExpression="<%$ resources:DataFieldRes,CustomerLastSODate %>">
                                            <ItemTemplate>
                                                <asp:Label ID="lblCustomerLastSODate" runat="server" Text="" ToolTip=""></asp:Label>
                                            </ItemTemplate>
                                            <ItemStyle Width="8%" />
                                            <HeaderStyle CssClass="numeric" />
                                        </asp:TemplateField>
                                        <asp:TemplateField SortExpression="<%$ resources:DataFieldRes,CustomerLastSOQty %>">
                                            <HeaderTemplate>
                                                <asp:Label ID="lblLastOrderQtyHdr" runat="server" Text='<%# GetLocalResourceObject("LastOrderQty").ToString()%>'
                                                    CssClass="numeric" />
                                            </HeaderTemplate>
                                            <ItemTemplate>
                                                <asp:Label ID="lblLastOrderQty" runat="server" ToolTip='<%# Eval(Resources.DataFieldRes.CustomerLastSOQty) %>'
                                                    Text='<%# Eval(Resources.DataFieldRes.CustomerLastSOQty) %>' />
                                            </ItemTemplate>
                                        ...
                                        <asp:TemplateField HeaderText="<%$ resources:MailSenton %>" SortExpression="<%$ resources:DataFieldRes,CustomerLastMailOn %>">
                                            <ItemTemplate>
