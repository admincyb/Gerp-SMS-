<%@ Page  Title="<%$ Resources:Captions,Title_PayrollProcess %>" Language="C#" AutoEventWireup="true" CodeBehind="FullandFinalSettlement.aspx.cs" 
    Inherits="HRMS.Payroll.FullandFinalSettlement" MasterPageFile="~/ERPSMS_2.Master"
    Theme="ClassicExt" MaintainScrollPositionOnPostback="true" ValidateRequest="false"%>
...
<asp:Content ID="Content1" runat="server" ContentPlaceHolderID="Head">
    ...
    <script type="text/javascript">
        ...
        function InitComponents() {
            ...
            GrandScriptUtils.DatePickerCommon("txtResignDate");
             ...
             if (controlID == "txtProcessFromDate") {
                 if ($("[id$=ddlProcessMode]").val() == $("[id$=hdfMonthlyMode]").val()) {
                    ...
                    // var toDateText = $.datepicker.formatDate("dd-M-yy", fromDate);
                     ...
                     var leavingdt = $("[id$=txtProcessDate]").val();
                 $("[id$=txtProcessToDate]").val(leavingdt);
                 $("[id$=hdfProcessToDate]").val(leavingdt);
                 }
                 else {
        ...
        function InitEmployeeAuto() {
            ...
            var branch1 = 0;
            var emptyp = 0;
            var employementtype = 0;
            
            if ($("[id$=ddlbranchlocation]").val() != 'undefined' && $("[id$=ddlbranchlocation]").val() != undefined) {

                branch1 = $("[id$=ddlbranchlocation]").val();
            }
            if ($("[id$=ddlEmployeeType]").val() != 'undefined' && $("[id$=ddlEmployeeType]").val() != undefined)
            {
                emptyp = $("[id$=ddlEmployeeType]").val();
            }
            if ($("[id$=ddlEmploymentType]").val() != 'undefined' && $("[id$=ddlEmploymentType]").val() != undefined)
            {
                employementtype = $("[id$=ddlEmploymentType]").val();
            }
            GrandScriptUtils.MakeAutoCompleteDDL("txtEmployee ", url + "?EmpCategory=2&EmpBranch=" + branch1 + "&EmpType=" + emptyp + "&EmploymentType=" +employementtype + "&EmpCompany=" + $("[id$=ddlCompany]").val() + "&EmpDept=" + $("[id$=hdfDepartment]").val() + "&FromDate=" + $("[id$=txtProcessFromDate]").val() + "&ToDate=" + $("[id$=txtProcessToDate]").val() + "&EmpPayrollType=" + $("[id$=ddlPayrollType]").val() + "&EmpCurrency=" + $("[id$=hdfCurrency]").val(), "hdfEmployee", true, true, "EMPLOYEEPAYROLLAUTOCOMPLETE");
            ResetEmployee();
        }
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <asp:UpdatePanel ID="upPayrollProcess" runat="server">
        <ContentTemplate>
                                    ...
                                    <td>
                                        <div class="div2col-S">
                                            ...
                                            <asp:RequiredFieldValidator ID="rfvProcessDate" CssClass="star" SetFocusOnError="true"
                                             ...
                                             <asp:Label ID="lblResignDate" runat="server" Text="<%$ resources:ResignDate%>"
                                                AssociatedControlID="txtResignDate" CssClass="middle-lbl-d"></asp:Label>
                                            <asp:TextBox ID="txtResignDate" runat="server" CssClass="input-small" TabIndex="1"
                                                onkeydown="return CheckKey(event)" MaxLength="11" onpaste="return false;"></asp:TextBox>
                                           <%-- <asp:RequiredFieldValidator ID="RequiredFieldValidator1" CssClass="star" SetFocusOnError="true"
                                                ValidationGroup="Process" EnableClientScript="true" runat="server" ControlToValidate="txtResignDate"
                                                Display="Dynamic" Text="*" ErrorMessage="<%$ resources:Err_ResignDate %>">
                                            </asp:RequiredFieldValidator>--%>
                                            <div class="clear">
                                            </div>
                                            ...
                                            <div class="gridwrap scroll-h300 margntop-minus3">
                                                <asp:GridView runat="server" ID="grdEmpPayrollList" Width="100%" AllowPaging="false"
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField HeaderText="<%$ resources:Gratuity%> " SortExpression="">
                                                            <ItemTemplate>
                                                                <asp:Label ID="lblGratuity" runat="server" Text='<%#GetFormattedCurrencyWithComma(Eval("EPS_GRATUITY"))%>'
                                                                    ToolTip='<%#GetFormattedCurrencyWithComma(Eval("EPS_GRATUITY"))%>'></asp:Label>
                                                            </ItemTemplate>
                                                            <ItemStyle CssClass="amount-numeric" />
                                                            <HeaderStyle Width="10%" CssClass="amount-numeric" />
                                                        </asp:TemplateField>
            ...
            <asp:HiddenField ID="hdfIsFinalize" Value="1" runat="server" />
        </ContentTemplate>
    </asp:UpdatePanel>
</asp:Content>
